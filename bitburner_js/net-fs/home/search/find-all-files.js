/** @typedef {{start:string;grep: string|null;_:string[]}} ScriptFlags */
...
export async function main(ns) {
	...
	do {
		...
		for (let i = 0; i < neighbors.length; i++) {
			...
			const filtered_dir = flags_.grep !== null ? remote_dir.filter(file => file.includes(flags_.grep)) : remote_dir;
