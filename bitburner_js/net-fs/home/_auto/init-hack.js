...
export async function main(ns) {
	...
	for (let [, , hostname] of server_map_arr) {
		const srv = server_map.get(hostname);
	...
	for (let [, [sz], hostname] of server_map_arr) {
		if (hostname.startsWith("big-")) continue;
		let srv1 = server_map.get(hostname);
		if (!srv1.hasAdminRights) continue;
		if (sz === 0) {
			if (trace) ns.printf(
				"[w:%s, b:%s lvl:%s %s ~/]> %s",
				ns.tFormat(async_delay),
				srv1.backdoorInstalled,
				srv1.requiredHackingSkill,
				hostname,
	...
	for (let [, , hostname] of server_map_arr) {
		if (hostname.startsWith("big-")) continue;
		let srv = server_map.get(hostname);
...
export function gen_server_crack(ns) {
	...
	const service_map = {
		ssh: x => ns.brutessh(x),
		ftp: x => ns.ftpcrack(x),
		smtp: x => ns.relaysmtp(x),
		http: x => ns.httpworm(x),
		sql: x => ns.sqlinject(x),
	}
