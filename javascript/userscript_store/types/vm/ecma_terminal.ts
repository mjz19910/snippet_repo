...
function lex_js(term_lexer: ecma_terminal, str: string) {
	...
	while(cur_index <= (str.length-1)) {
		...
		console.log(str.slice(cur_index-2, cur_index+1));
...
function to_log_return(code:string, res: ecma_return_type) {
	...
	return ` @@End(json\`[${res.map(null_to_str).join(",")}]\`)`;
}
...
function run_tests_impl() {
	let test_1 = {
		code:`(function(){let the_var=12;})`
	};
	let test_2 = {
		code:`(class {#name=12;})`
	};
	let js_r1 = eval(test_1.code);
	let js_r2 = eval(test_2.code);
	void js_r1;
	void js_r2;
	...
	let log_res=null;
	// Test 1 (test_1_code)
	res_arr=lex_js(term_lexer, test_1.code);
	cur_index=0;
	log_res=res_arr.map(to_log_return.bind(null, test_1.code));
	let log_fmt=res_arr.map(()=>"%s").join("");
	console.log('test_ecma_12_6 res_arr for test_1 '+log_fmt, ...log_res);
	// Test 2 (test_2_code)
	res_arr=lex_js(term_lexer, test_2.code);
	cur_index=0;
	log_res=res_arr.map(to_log_return.bind(null, test_2.code));
