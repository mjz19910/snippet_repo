...
exports.AutoBuy = exports.AutoBuyState = exports.window = exports.DocumentWriteList = exports.RemoteWorkerState = void 0;
const ArrayBox_1 = require("../types/vm/ArrayBox");
const WindowBox_1 = require("../types/vm/WindowBox");
const GlobalThisBox_1 = require("../types/vm/GlobalThisBox");
const IStackVMBox_1 = require("../types/vm/IStackVMBox");
const IndexedObject_1 = require("../types/vm/IndexedObject");
const IndexedFunctionBox_1 = require("../types/vm/IndexedFunctionBox");
const InstructionTypeArrayBox_1 = require("../types/vm/InstructionTypeArrayBox");
function fire_timer(timer, remote_id) {
    timer.fire(remote_id);
}
class RemoteTimer {
    m_remote_to_local_timer_state_map;
    m_api_info;
    base_id;
...
class RemoteWorkerState {
    ...
    timer_set(type, remote_id, timeout) {
        if (this.m_timer)
...
exports.RemoteWorkerState = RemoteWorkerState;
...
exports.DocumentWriteList = DocumentWriteList;
...
function worker_code_function(verify_callback) {
    ...
    class RemoteWorkerState {
        ...
        timer_set(timer_type_tag, remote_id, timeout) {
            if (this.m_timer)
...
class WorkerState {
    ...
    static has_global_state() {
        return exports.window.hasOwnProperty("worker_state");
    }
    ...
    static get_global_state() {
        return exports.window[this.global_state_key];
    }
    static set_global_state(worker_state_value) {
        exports.window[this.global_state_key] = worker_state_value;
    }
    static delete_global_state() {
        delete exports.window[this.global_state_key];
    }
...
class Timer {
    ...
    constructor(api_info) {
        ...
        if (api_info.set_single)
            this.m_api_map.set(api_info.set_single, exports.window[api_info.set_single]);
        if (api_info.set_repeating)
            this.m_api_map.set(api_info.set_repeating, exports.window[api_info.set_repeating]);
        if (api_info.clear_single)
            this.m_api_map.set(api_info.clear_single, exports.window[api_info.clear_single]);
        if (api_info.clear_repeating)
            this.m_api_map.set(api_info.clear_repeating, exports.window[api_info.clear_repeating]);
        if (api_info.set_single && api_info.clear_single) {
            this.base_id = exports.window[api_info.set_single](timer_nop);
            exports.window[api_info.clear_single](this.base_id);
        }
        else if (api_info.set_repeating && api_info.clear_repeating) {
            this.base_id = exports.window[api_info.set_repeating](timer_nop);
            exports.window[api_info.clear_repeating](this.base_id);
        }
        else {
    ...
    set(type, target_function, delay, target_arguments) {
    ...
    destroy() {
        ...
        if (api_info.set_single)
            exports.window[api_info.set_single] = api_map.get(api_info.set_single);
        if (api_info.set_repeating)
            exports.window[api_info.set_repeating] = api_map.get(api_info.set_repeating);
        if (api_info.clear_single)
            exports.window[api_info.clear_single] = api_map.get(api_info.clear_single);
        if (api_info.clear_repeating)
            exports.window[api_info.clear_repeating] = api_map.get(api_info.clear_repeating);
...
function move_timers_to_worker_promise_executor(executor_accept, executor_reject) {
    ...
    function remoteSetTimeout(handler, timeout = 0, ...target_arguments) {
        if (!worker_state) {
            exports.window.setTimeout = setTimeout_global;
    ...
    function remoteClearTimeout(id) {
        if (!worker_state) {
            exports.window.clearTimeout = clearTimeout_global;
            console.log('lost worker_state in timer');
    ...
    function remoteSetInterval(handler, timeout = 0, ...target_arguments) {
        if (!worker_state) {
            exports.window.setInterval = setInterval_global;
            console.log('lost worker_state in timer');
    ...
    function remoteClearInterval(id) {
        if (!worker_state) {
            exports.window.clearInterval = clearInterval_global;
            console.log('lost worker_state in timer');
    ...
    exports.window.setTimeout = remoteSetTimeout;
    exports.window.setInterval = remoteSetInterval;
    exports.window.clearTimeout = remoteClearTimeout;
    exports.window.clearInterval = remoteClearInterval;
    return {
        get() {
...
class EventHandlerDispatch {
    target_obj;
    target_fn;
    constructor(target_obj, target_fn) {
...
class SimpleStackVMParser {
    ...
    static raw_parse_handle_regexp_match(match_parts) {
        if (!match_parts)
            return;
        ...
        while (this.instruction_pointer < this.instructions.length && this.running) {
            ...
            switch (cur_opcode) {
                case 'push' /*Stack*/: {
                    for (let i = 1; i < cur_instruction.length; i++) {
                        ...
                        if (item instanceof Array) {
                            this.push(new InstructionTypeArrayBox_1.InstructionTypeArrayBox(item));
                        }
                        else {
                            this.push(item);
                        }
                ...
                case 'get' /*Object*/: {
                    ...
                    if (!obj)
                        throw new Error("Invalid");
                    if (obj instanceof IndexedObject_1.IndexedObject && typeof name === 'string') {
                ...
                case 'call' /*Call*/: {
                    ...
                    if (!name_to_call)
                        throw "Bad";
                    if (target instanceof IndexedFunctionBox_1.IndexedFnBox && typeof name_to_call === 'string') {
                ...
                case 'push_args' /*Special*/:
                    this.push(new ArrayBox_1.ArrayBox(run_arguments));
                    break;
                case 'this' /*Special*/:
                    this.push(new IStackVMBox_1.IStackVMBox(this));
                    break;
                case 'global' /*Special*/: {
                    if (exports.window)
                        this.push(new WindowBox_1.WindowBox(exports.window));
                    else
                        this.push(new GlobalThisBox_1.GlobalThisBox(globalThis));
                    break;
                }
...
class AverageRatio {
    ...
    weight;
    human_duration;
    constructor(max_len, max_history_len, weight, human_duration, arr = []) {
        this.arr = arr;
        this.history = [];
        this.count = 0;
        this.len = max_len;
...
class AsyncDelayNode {
    ...
    label;
    timeout;
    constructor(root, target_obj, target_callback, label) {
        this.root = root;
        this.cint = -1;
        this.target_obj = target_obj;
...
class AutoBuyState {
    ...
    prev_atomepersecond;
    init() {
        if (exports.window.atomepersecond === 0) {
            new AsyncDelayNode(this.root_node, this, this.init, 'not ready AutoBuyState.update').start();
            return;
        }
        this.val = exports.window.totalAtome / exports.window.atomepersecond;
        let rep_val = this.val / (100 * 4 * exports.window.prestige);
        if (Number.isFinite(rep_val)) {
            for (let i = 0; i < 8; i++) {
                this.arr.push(rep_val * .75);
            }
        }
        this.prev_atomepersecond = exports.window.atomepersecond;
    ...
    div;
    update() {
        this.ratio_mult = exports.window.prestige;
        this.div = 60 * this.ratio_mult * 8;
        if (exports.window.atomepersecond === 0) {
            new AsyncDelayNode(this.root_node, this, this.update, 'not ready AutoBuyState.update').start();
            return;
        }
        this.val = exports.window.totalAtome / exports.window.atomepersecond / this.div;
        if (!Number.isFinite(this.val)) {
            console.log('fail', this.div, exports.window.atomepersecond, exports.window.totalAtome);
            new AsyncDelayNode(this.root_node, this, this.update, 'not ready AutoBuyState.update').start();
            return;
        }
...
exports.AutoBuyState = AutoBuyState;
class AutoBuy {
    delay;
    extra;
    iter_count;
    ...
    get_delay_arr_data(forced_action) {
        if (forced_action == "RESET")
            return this.delay_arr.map((e) => ~~(e / 4)).join(",");
    ...
    history_element;
    delay_element;
    hours_played_element;
    percent_ratio_element;
    percent_ratio_change_element;
    state_log_element;
    dom_pre_init() {
        let style_string = "";
    ...
    init_dom() {
        ...
        exports.window.addEventListener('unload', function () {
            t.save_state_history_arr();
            t.save_delay_arr();
        });
    }
    global_init() {
        if (exports.window.g_auto_buy && exports.window.g_auto_buy !== this)
            exports.window.g_auto_buy.destroy();
        exports.window.g_auto_buy = this;
    }
    destroy() {
    ...
    update_dom() {
        this.hours_played_element.innerText = ((exports.window.timeplayed / 30) / 60).toFixed(3) + " hours";
    ...
    delayed_init() {
        ...
        this.update_dom();
        this.main();
        let original_lightreset = exports.window.lightreset;
        exports.window.lightreset = lightreset_inject;
        exports.window.specialclick = specialclick_inject;
    ...
    get_updated_delay(change, decrease) {
        let value;
        if (decrease)
    ...
    do_delay_dec(pow_terms, div) {
        ...
        let delay_change = this.get_delay_change(pow_terms[0], Math.log(exports.window.totalAtome), div);
    ...
    is_special_done(special_buyable) {
        return !special_buyable.done && special_buyable.cost < exports.window.totalAtome;
    }
    next_special() {
        return exports.window.allspec.findIndex(this.is_special_done);
    }
    do_special() {
        let ret = false;
        for (let index = this.next_special();; index = this.next_special()) {
            if (index > -1) {
                exports.window.specialclick(index);
                ret = true;
            }
            else
                break;
...
class DoUnitPromoteExecutor {
    run() {
        var out = [], maxed = [];
        for (var k = 0; k < exports.window.arUnit.length; k++) {
            var afford = false;
            if (exports.window.arUnit[k][16] == true || k == 0) {
                var type = exports.window.Get_Unit_Type(k);
                var tmp = exports.window.getUnitPromoCost(k);
                var cost = tmp;
                var next = exports.window.Find_ToNext(k);
                if (next < 0) {
                    maxed[k] = true;
                }
                ;
                for (var i = 1; i <= 100; i++) {
                    if (exports.window.totalAtome >= cost) {
                        tmp = tmp + (tmp * exports.window.arUnit[k][3]) / 100;
                        var tar = (exports.window.arUnit[k][4] * 1) + i;
                        var a = exports.window._targets.indexOf(tar);
                        var reduction = 1;
                        if (a > -1 && tar <= 1000) {
                            var b = true;
                            for (var k2 in type[2]) {
                                var v2 = type[2][k2];
                                if (v2 != k && exports.window.arUnit[v2][4] < tar) {
                                    b = false;
                                }
                            }
                            if (b) {
                                var c = exports.window._targets_achi.indexOf(exports.window.totalAchi() + 1);
                                if (c > -1) {
        ...
        if (maxed[res]) {
            for (var y = 0; y < 100; y++) {
                exports.window.mainCalc(res);
            }
        }
        else {
            exports.window.tonext(res);
        }
    }
}
...
function specialclick_inject(that) {
    let allspec = exports.window.allspec;
    let doc = exports.window.doc;
    if (allspec[that].done == undefined)
        allspec[that].done = false;
    if (allspec[that].cost <= exports.window.totalAtome && allspec[that].done == false) {
        let sb = doc.getElementById('specialsbought');
        if (!sb)
            throw new Error;
        sb.innerText = exports.window.rounding(++exports.window.specialsbought, false, 0);
        if (that == 74) {
        }
        exports.window.atomsinvest += allspec[that].cost;
        let ae = doc.getElementById('atomsinvest');
        if (ae !== void 0 && ae !== null)
            ae.innerText = exports.window.rounding(exports.window.atomsinvest, false, 0);
        allspec[that].done = true;
        exports.window.totalAtome -= allspec[that].cost;
        var diff1 = exports.window.calcDiff(that);
        for (var a in exports.window.arUnit[that][17])
            exports.window.arUnit[that][17][a] *= 100;
        exports.window.arUnit[that][5] *= 100;
        var specaps = 0;
        if (exports.window.arUnit[that][4] > 0) {
            specaps = (exports.window.calcDiff(that) - diff1);
            exports.window.atomepersecond += specaps;
        }
        if (exports.window.noti)
            exports.window.gritter('Power-up !', exports.window.toTitleCase(exports.window.plurials(exports.window.arrayNames[that])) + " X100 APS", null, "+" + exports.window.rounding(specaps, false, 0) + " APS", "");
        exports.window.updateprogress(that);
        exports.window.$('#spec' + that).remove();
        (that < 74) ? exports.window.seeUnit(that + 1) : exports.window.seeUnit(that - 1);
        exports.window.seeUnit(that);
        exports.window.checkspec();
        exports.window.achiSpec();
    }
}
function on_page_is_loaded() {
    remove_bad_dom_script_element();
    if (exports.window.Pace.bar.progress == 100) {
        auto_buy_obj.init();
    }
    else {
        let original_pace_bar_finish = exports.window.Pace.bar.finish;
        exports.window.Pace.bar.finish = function () {
...
function define_property_value(obj, name, value, ...props) {
    let [writable = true, enumerable = true, configurable = true] = props;
...
function got_jquery(jquery_func) {
    define_property_value(exports.window, '$', jquery_func);
...
function pace_finish_proxy_apply(func, this_v, args) {
    auto_buy_obj.init();
    exports.window.Pace.bar.finish = func;
    if (args.length > 0) {
...
function on_game_data_set() {
    ...
    exports.window.Pace.bar.finish = new Proxy(exports.window.Pace.bar.finish, {
...
function main() {
    ...
    exports.window.cint_arr = cint_arr;
    if (enable_proxy) {
        proxy_jquery();
    }
    exports.window.adsbygoogle = [];
    exports.window.adsbygoogle.op = exports.window.adsbygoogle.push;
    exports.window.adsbygoogle.push = function (e) {
        exports.window.adsbygoogle.op(e);
        remove_bad_dom_script_element();
    };
    ...
    remove_bad_dom_script_element();
    exports.window.on_on_timers_moved_first = true;
    ...
    exports.window.document_write_list = new DocumentWriteList;
