	...
	ecmascript: {
		...
		class ecma_12_3 {
			...
			LineTerminator(str, index) {
				if(str[index] === '\r') return 1;
				if(str[index] === '\n') return 1;
				//<LS>
				if(str[index] === '\u{2028}') return 1;
				//<PS>
				if(str[index] === '\u{2029}') return 1;
				return 0;
			}
		...
		class ecma_12_5 {
			...
			*/
			CommonToken(str, index) {
				let len = 0;
				if((len = this.IdentifierName(str, index)) > 0) return len;
				if((len = this.PrivateIdentifier(str, index)) > 0) return len;
				if((len = this.Punctuator(str, index)) > 0) return len;
				if((len = this.NumericLiteral(str, index)) > 0) return len;
				if((len = this.StringLiteral(str, index)) > 0) return len;
				if((len = this.Template(str, index)) > 0) return len;
			}
		}
		class ecma_12_6 {
			static _attach(root) {
