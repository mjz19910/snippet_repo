...
const calcDepFlags=(/** @type {{ dev: boolean; optional: boolean; devOptional: boolean; peer: boolean; }} */ tree,resetRoot=true) => {
  ...
  const ret=depth({
    tree,
    visit: (/** @type {any} */ node) => calcDepFlagsStep(node),
    filter: (/** @type {any} */ node) => node,
    /** @param {any} node */
    leave(node) {
      if(debug) console.log('leave',util.formatWithOptions({colors: true,depth: 3,compact: 1},node));
    },
    getChildren: (/** @type {any} */ node,/** @type {{ edgesOut: any[]; }} */ tree) => {
...
/** @return {any} */
const calcDepFlagsStep=(/** @type {{ [x: string]: any; extraneous?: any; target?: any; isLink?: any; dev?: any; optional?: any; devOptional?: any; peer?: any; edgesOut?: any; }} */ node) => {
  ...
  node.edgesOut.forEach((/**@type {{peer:{},optional:{},dev:{},to:{extraneous:boolean}}} */{peer,optional,dev,to}) => {
...
const resetParents=(/** @type {{ [x: string]: any; }} */ node,/** @type {string} */ flag) => {
  if(node[flag]) {
    return;
  }
...
// have the flag set.
const unsetFlag=(/** @type {{ [x: string]: boolean; }} */ node,/** @type {string} */ flag) => {
  if(node[flag]) {
    node[flag]=false;
    depth({
      tree: node,
      visit: (/** @type {{ [x: string]: boolean; extraneous: boolean; target: { [x: string]: boolean; extraneous: boolean; }; isLink: any; }} */ node) => {
      ...
      getChildren: (/** @type {{ target: { edgesOut: any[]; }; }} */ node) => {
