...
function make_includes(t, u) {
	/**@type {{}} */
	let x = t;
	let cc = Object.keys(u);
	console.log(cc);
	if(type_verify_includes(t, u, x)) {
		return x;
	}
	return null;
}
/**@type {<T, B>(t:T, b:B, inc_cls:any)=>inc_cls is T&B} */
function type_verify_includes(cls_a, cls_b, inc_cls) {
	cls_a;
	cls_b;
	inc_cls;
	return true;
...
function use_constants(){
	return [
		tmp_idl,
	]
}

function use_type_helpers(){
	return [
		make_includes,
	]
}