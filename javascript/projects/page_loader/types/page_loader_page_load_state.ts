...
class FetchRequestState {
	...
	async on_incoming_message_result(page_content:Uint8Array | null) {
		if(!fake.document)
	...
	async on_incoming_message(message:IncomingMessage) {
		if(!fake.document)
			throw new Error("Missing document")
	...
	url: string|null;
	m_start_request_module: {
		get(url:{},cb:(x:IncomingMessage)=>void):ClientRequest;
	}|null=null;
	m_client_request:ClientRequest|null=null;
	m_incoming_message:IncomingMessage|null=null;
	/**
	 * @arg {string | null} url
