...
delete errn;
[sz, errn] = (function() {
	var sortorder = ["function", "object", "string", "number", "symbol", "boolean", "undefined"];
	var toadd = [];
	...
	var i;
	var by_of = (a, b) => {
		return sortorder.indexOf(typeof a) - sortorder.indexOf(typeof b);
	};
	...
	while(st.dep > 0) {
		fnx("s.r", a, st);
		console.log(cnow - performance.now(), a.size);
	}
	console.log("done_8");
	seen = [];
	st.dep = 3;
	...
	var lm = a;
	delete a;
	var tmpa = [seen.length, lm, ges, seen];
	return tmpa;
}
)();
