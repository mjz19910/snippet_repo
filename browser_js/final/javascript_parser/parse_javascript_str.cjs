...
class NumericLiterals extends ECMA262Base {
	...
	/** @arg {string} str @arg {number} index @returns {LexReturnTyShort} */
	LegacyOctalIntegerLiteral(str, index) {
		str;
		index;
		throw new Error("No impl");
	}
	...
	/** @arg {string} str @arg {number} index @returns {LexReturnTyShort} */
	NonOctalDecimalIntegerLiteral(str, index) {
		str;
		index;
		throw new Error("No impl");
	}
	...
	/** @arg {string} str @arg {number} index @returns {LexReturnTyShort} */
	LegacyOctalLikeDecimalIntegerLiteral(str, index) {
		str;
		index;
		throw new Error("No impl");
	}
...
class StringLiterals extends ECMA262Base {
	...
	LineContinuation(str, index) {
		if (str[index] === "\\") {
			const [, , lt_len] = this.parent.line_terminators.LineTerminatorSequence(
				str,
				index + 1,
			);
			if (lt_len > 0) {
	...
	UnicodeEscapeSequence(str, index) {
		...
		if (str[index + off] === "{}"[0]) {
			off++;
			const len = this.parent.template_literal_lexical_components.CodePoint(
				str,
				index + off,
			);
			if (len[2] > 0) {
...
class TemplateLiteralLexicalComponents extends ECMA262Base {
	...
	NotEscapeSequence(str, index) {
		...
		let res_1, res_2;
		let len = 1;
		let lookahead_res_1 = this.parent.numeric_literals.HexDigit(
			str,
			index + len,
		);
		...
		lookahead_res_1 = this.parent.numeric_literals.HexDigit(
			str,
			index + len + 1,
		);
...
class RegularExpressionLiterals extends ECMA262Base {
	...
	RegularExpressionFlags(str, index) {
		// [empty]
		const len = 0;
		const is_class_chars = this.parent.names_and_keywords.IdentifierPartChar(
			str,
			index + len,
		);
		if (!is_class_chars[0]) {
...
class js_token_generator {
	...
	ParseLineTerminator(in_state, out_state) {
		const res = this.root.line_terminators.LineTerminator(
			in_state.str,
			in_state.index,
		);
	...
	ParseRightBracePunctuator(in_state, out_state) {
		const res = this.root.punctuators.RightBracePunctuator(
			in_state.str,
			in_state.index,
		);
	...
	ParseDivPunctuator(in_state, out_state) {
		const res = this.root.punctuators.DivPunctuator(
			in_state.str,
			in_state.index,
		);
	...
	ParseRegularExpressionLiteral(in_state, out_state) {
		const res = this.root.RegularExpressionLiterals.RegularExpressionLiteral(
			in_state.str,
			in_state.index,
		);
	...
	ParseTemplateSubstitutionTail(in_state, out_state) {
		const res = this.root.template_literal_lexical_components
			.TemplateSubstitutionTail(in_state.str, in_state.index);
