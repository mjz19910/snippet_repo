...
class StringLiterals extends ECMA262Base {
	...
	LineContinuation(str, index) {
		if (str[index] === "\\") {
			let [, , lt_len] = this.parent.line_terminators.LineTerminatorSequence(
				str,
				index + 1,
			);
			if (lt_len > 0) {
	...
	UnicodeEscapeSequence(str, index) {
		...
		if (str[index + off] === "{}"[0]) {
			off++;
			let len = this.parent.template_literal_lexical_components.CodePoint(
				str,
				index + off,
			);
			if (len[2] > 0) {
...
class RegularExpressionLiterals extends ECMA262Base {
	...
	RegularExpressionFlags(str, index) {
		// [empty]
		let len = 0;
		let is_class_chars = this.parent.names_and_keywords.IdentifierPartChar(
			str,
			index + len,
		);
		if (!is_class_chars[0]) {
...
class js_token_generator {
	...
	ParseLineTerminator(in_state, out_state) {
		let res = this.root.line_terminators.LineTerminator(
			in_state.str,
			in_state.index,
		);
	...
	ParseRightBracePunctuator(in_state, out_state) {
		let res = this.root.punctuators.RightBracePunctuator(
			in_state.str,
			in_state.index,
		);
	...
	ParseRegularExpressionLiteral(in_state, out_state) {
		let res = this.root.RegularExpressionLiterals.RegularExpressionLiteral(
			in_state.str,
			in_state.index,
		);
	...
	ParseTemplateSubstitutionTail(in_state, out_state) {
		let res = this.root.template_literal_lexical_components
