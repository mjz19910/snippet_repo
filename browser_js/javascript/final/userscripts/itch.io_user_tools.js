...
(function() {
	...
	document.addEventListener("DOMContentLoaded",function() {
		var user_tools_element=document.getElementById("user_tools");
		if(!user_tools_element) return;
		var number_of_tools=user_tools_element.children.length;
		...
		child_element.onclick=function() {
			...
			if(child_element.getAttribute("mode")=='h') {
				Array.prototype.slice.call(tools.children,0,number_of_tools).forEach(function(e) {e.style.display='none';});
				child_element.firstChild.innerText="Show";
				child_element.setAttribute("mode","s");
			} else {
				Array.prototype.slice.call(tools.children,0,number_of_tools).forEach(function(e) {e.style.display='';});
				child_element.firstChild.innerText="Hide";
				child_element.setAttribute("mode","h");
			}
		};
		user_tools_element.appendChild(list_item_element);
	});
	// Your code here...
})();