...
const page_require = typeof require === "undefined"
	? __module_require__
	: require;
let delete_require = false, reset_require = false;
...
function fetch_all_images() {
	return Promise.all(
		window.arUnit.slice(0, -1).map((e) => "images/" + e[11]).map(async (e) => {
			try {
...
function fetch_all_images_full() {
	return window.arrayNames.map((e) =>
		e.indexOf("cat") > -1 ? "cats-eye-nebula" : e
	).map((e) => "imagesFull/" + e.replace(/\s+/g, "-") + ".jpg").map((e) =>
		fetch(e)
	);
}
...
function fetch_all_specs() {
	return window.allspec.map((_e, i) => "specs/" + (i + 1) + ".jpg").map((e) =>
		fetch(e)
	);
}
fetch_all_specs;
...
function append_console_message(level, format_str, ...args) {
	...
	if (level_str !== "unknown") format_str = "[%s] " + format_str;
	else format_str = "[%o:%s] " + format_str;
	if (level >= LOG_LEVEL_ERROR_IMPL) {
		if (LogErrorAsConsoleError) console.error(format_str, level_str, ...args);
		else console.info(format_str, level_str, ...args);
...
function human_log_level(level) {
	switch (level) {
		case LOG_LEVEL_CRIT_IMPL:
			return "crit";
		case LOG_LEVEL_ERROR_IMPL:
			return "error";
		case LOG_LEVEL_WARN_IMPL:
			return "warn";
		case LOG_LEVEL_NOTICE_IMPL:
			return "notice";
		case LOG_LEVEL_INFO_IMPL:
			return "info";
		case LOG_LEVEL_DEBUG_IMPL:
			return "debug";
		case LOG_LEVEL_TRACE_IMPL:
			return "trace";
		default:
			return "unknown";
	}
...
function update_logger_vars() {
	...
	if (sessionStorage["LogErrorAsConsoleError"]) {
		LogErrorAsConsoleError =
			sessionStorage["LogErrorAsConsoleError"] === "true";
	}
...
/** @typedef {import("./ns.ts").CSSStyleSheetBox} CSSStyleSheetBox_CJS */
...
/** @typedef {import("./ns.ts").StackVMBox} StackVMBox_CJS */
...
/** @typedef {import("./ns.ts").WindowBox} WindowBox_CJS */
...
class InstructionCallImpl extends InstructionImplBase {
	...
	handle_as_fn_box(vm, fn_box, target_this, arg_arr) {
		if ("return_type" in fn_box) {
			switch (fn_box.return_type) {
				case "Promise<Box>":
					return this.handle_as_fn_to_promise(
						vm,
						fn_box.value,
						target_this,
						arg_arr,
	...
	run(vm, number_of_arguments) {
		...
		if (value_box === void 0) {
			console.info(
				"VM Error: Processing call instruction",
				number_of_arguments,
				target_this,
				value_box,
				arg_arr,
...
class InstructionConstructImpl extends InstructionImplBase {
	...
	run(vm, number_of_arguments) {
		...
		log_if_impl_r(
			...
			"",
			number_of_arguments,
			...vm.stack.slice(vm.stack.length - number_of_arguments),
		);
	}
}
	...
	run(vm, cast_type) {
		...
		switch (cast_type) {
			case "object_index":
				break;
			default:
				throw new Error("Missing cast to " + cast_type);
		}
...
/** @typedef {import("./ns.ts").NumberBox} NumberBox_CJS */
...
class InstructionPushImpl extends InstructionImplBase {
	/** @type {"push"} */
	type = "push";
	/** @arg {StackVMImpl} vm @arg {this["type"]} _ @arg {import("./ns.ts").Box[]} args */
	run(vm, _, ...args) {
		vm.stack.push(...args);
	}
}
			...
			case "constructor_box":
				{
					switch (value_box.instance_type) {
						case "CSSStyleSheet":
							new CSSStyleSheetConstructorBoxImpl(value_box.value).on_get(
								vm,
								key,
							);
							break;
						case "unknown":
							{
								new NewableFunctionBoxImpl(
									...value_box.get_construct_arguments(),
								);
							}
							break;
					}
				...
				break;
			default:
				console.log("on_get no handler", value_box.type);
		}
	}
	/** @arg {StackVMImpl} vm */
	run(vm) {
...
class InstructionReturnImpl extends InstructionImplBase {
	...
	/** @arg {StackVMImpl} vm */
	run(vm) {
		if (vm.stack.length > 0) vm.return_value = vm.pop();
...
class InstructionAppendImpl extends InstructionImplBase {
	...
	run(vm) {
		...
		if (
			!(append_obj.type === "instance_box" &&
				append_obj.instance_type === "Node")
		) throw new Error("1");
		if (!(target.type === "instance_box" && target.instance_type === "Node")) {
			throw new Error("1");
		}
...
/** @type {typeof import("./as_const.ts").instruction_descriptor_arr_type} */
...
class StackVmBaseImpl {
	...
	update_instruction(offset, value, lex_instruction) {
		...
		} else if (offset > 0) lex_instruction[offset] = value;
		else throw new Error("Unreachable");
		return lex_instruction;
	}
}

/** @typedef {import("./support/InstructionType.ts").InstructionType} InstructionType_CJS */
/** @typedef {import("./ns.ts").Box} Box_CJS */
/** @typedef {import("./ns.ts").StackVM} StackVM_CJS */
...
class StackVMImpl {
	...
	run() {
		...
		if (this.stack.length !== 0) {
			...
			console.assert(
				false,
				"stack length is not zero, unhandled data on stack",
			);
		}
...
class StackVMParserImplR {
	...
	static parse_string_into_raw_instruction_stream(string) {
		...
		if (parts) {
			console.assert(
				false,
				"StackVM Parser: Iteration limit exceeded (limit=%o)",
				parser_max_match_iter,
			);
		}
		return arr;
	}
...
class DocumentWriteListImpl {
	...
	destroy(should_try_to_destroy = false) {
		if (this.attached_document && this.document_write_proxy) {
			console.assert(
				this.attached_document.write === this.document_write_proxy,
			);
...
class IntervalNode extends BaseNodeImpl {
	...
	/** @arg {{}|null} target */
	start(target = null) {
		if (target) this.m_target = target;
...
/** @typedef {import("./ns.ts").VoidBox_ForUndefined} VoidBox_CJS */
...
class AutoBuyStateImplR {
	...
	on_ratio_change(do_lock, dir_num, lock_for) {
		if (do_lock) this.do_ratio_lock(do_lock, dir_num, lock_for);
		else this.do_ratio_lock(do_lock, dir_num, lock_for);
	...
	log_on_update_ratio_mode_notify() {
		log_if_impl_r(
			LOG_LEVEL_INFO_IMPL,
			"update_ratio_mode_tag mode=%o total_mul=%o cycle_change=%o",
			this.ratio_mode,
			this.total_mul,
		...
		if (exponent < 2 && exponent > -3) {
			log_if_impl_r(
				LOG_LEVEL_ERROR_IMPL,
				"update_ratio_mode_tag -exp avg:%s=%o lcc=%o",
				near_avg,
				(~~(real_val * 100000)) / 100000,
				this.locked_cycle_count,
			);
		} else {log_if_impl_r(
				LOG_LEVEL_ERROR_IMPL,
				"update_ratio_mode_tag +exp avg:%s=(%o,%o) lcc=%o",
				near_avg,
				(~~(num * 10000)) / 10000,
				exponent,
				this.locked_cycle_count,
	...
	update() {
		...
		this.val = Math.log2(window.totalAtome / window.atomepersecond / 2) /
			this.div;
...
class AsyncAutoBuy {
		...
		try {
			...
			while (this.main_running) {
				...
				for (let exit_loop = false; !exit_loop;) {
					...
					this.unit_upgradeable_trigger = 30;
					if (
						this.parent.timeout_ms && this.parent.timeout_ms > 3 * 60 * 1000
					) this.unit_upgradeable_trigger = 8;
					if (
						this.parent.unit_upgradable_count > this.unit_upgradeable_trigger
					) {
	...
	async fast_unit_async() {
		...
		while (this.fast_unit_running) {
			...
			await this.next_timeout_async(
				this.parent.round(this.parent.timeout_ms / 2),
				":",
			);
			count++;
...
class AutoBuyImplR {
	...
	test_log(log_level, format_str, ...args) {
		if (args.length > 0) args.unshift("test:");
	...
	pre_init() {
		...
		if (!(audio_element instanceof HTMLAudioElement)) {
			throw new Error(
				'querySelector("#background_audio") is not an instance of HTMLAudioElement',
			);
		}
	...
	save_timeout_arr() {
		...
		if (action_count !== void 0) {
			...
			if (Number.isFinite(action_count)) {
				if (action_count > 0) {
					localStorage["auto_buy_forced_action"] = [
						forced_action,
						action_count - 1,
					];
	...
	do_zero_pad(value, pad_char, char_num) {
		let string;
		if (typeof value === "number") string = value.toString();
		else string = value;
	...
	get_millis_as_pretty_str(timeout_milli, milli_acc) {
		...
		int_hours === 0 &&
			(time_arr.shift(),
				int_minutes === 0 &&
				(time_arr.shift(), int_seconds === 0 && time_arr.shift()));
	...
	set_secondinterval() {
		...
		this.root_node.append_child(
			new IntervalIdNodeRef(interval_id, function () {
	...
	state_history_append(value, silent = false) {
		...
		if (silent) {
			log_if_impl_r(
				LOG_LEVEL_DEBUG_IMPL,
				"state_history_append_tag silent item",
				value,
			);
			return;
		}
	...
	set_auto_buy_timeout() {
		if (this.timeout_ms) this.timeout_ms *= 0.9;
		else this.timeout_ms = 2;
	...
	calc_timeout_ms() {
		...
		log_if_impl_r(
			LOG_LEVEL_INFO_IMPL,
			"calc_timeout_ms sorted_diff index",
			zero_idx,
			"diff is",
			this.round(diff * diff_want_mul) / diff_want_mul,
		);
		log_if_impl_r(
			LOG_LEVEL_INFO_IMPL,
			"calc_timeout_ms l_diff %o %o\n%o",
			ez_log.slice(0, 8),
			ez_log.slice(-8),
	...
	start_main_async(no_wait = false) {
		return this.with_async.do_start_main_async(no_wait).then(
			(_e) => {},
			(e) => {
				console.log("err", e);
	...
	do_timeout_dec(pow_terms, div) {
		...
		log_if_impl_r(
			LOG_LEVEL_DEBUG_IMPL,
			"do_timeout_dec_tag dec timeout_change",
			change,
		);
	...
	do_timeout_inc(pow_terms, div) {
		...
		log_if_impl_r(
			LOG_LEVEL_DEBUG_IMPL,
			"do_timeout_inc_tag inc timeout_change",
			"change",
			change,
			"pow",
			pow_terms[1],
	...
	maybe_run_reset() {
		...
		switch (count) {
			...
			case 3:
				break;
			default:
				console.log("maybe_run_reset count", count);
		}
		if (
			this.state.ratio > 1 &&
			// this.is_epoch_over() &&
	...
	do_game_reset() {
		...
		this.next_timeout(
			this.game_reset_finish,
			this.round(this.timeout_ms / 3),
			"1R",
		);
		this.on_repeat_r();
	}
	...
	on_game_reset_finish(time_played) {
		...
		if (exponent <= -2 || exponent >= 2) {
			console.info("p_calc_1:expected prestige (%o,%o)%%", near_val, exponent);
				...
				"p_calc_2:expected prestige %o%%",
				(~~(real_val * prestige_acc)) / prestige_acc,
			);}
		window.lightreset();
	}
	on_repeat_r() {
...
function specialclick_inject(that) {
	...
	if (allspec[that].cost <= totalAtome && allspec[that].done == false) {
		...
		if (noti) {
			gritter(
				"Power-up !",
				toTitleCase(plurials(arrayNames[that])) + " X100 APS",
				null,
				"+" + rounding(spec_aps, false, 0) + " APS",
				"",
			);
		}
...
function remove_html_nodes(node) {
	...
	if (!node.src) return;
	if (node.src.includes("analytics.js") && node.src.includes("google")) {
		return node.remove();
	}
	...
	if (node.src.indexOf("ads") > -1 || node.src.indexOf("track") > -1) {
		return node.remove();
	}
}
...
function wait_for_game_data() {
	if (window._SM_Data) on_game_data_set();
...
function main() {
	...
	async function do_fetch_load() {
		...
		x: {
			...
			await new Promise(function (a) {
				...
				document.writeln(
					`<head></head><body><a href onclick="g_do_load()">load with fetch</a></body>`,
				);
				reset_global_event_handlers();
				document.close();
			});
		...
		let rb_html_tmp = rb_html.replace(
			/https:\/\/apis.google.com\/js\/platform.js/,
			"",
		);
		//spell:disable-next-line
		rb_html_tmp = rb_html_tmp.replace(
			"//script.opentracker.net/?site=rebuildtheuniverse.com",
			"",
		);
		let rc = 0;
		let did_rep = true;
		...
		window.onbeforeunload = function () {
			...
			if (history.state?.gen !== void 0 && history.state.prev === void 0) {
				...
				history.replaceState(
					{ prev: history.state, gen: history.state.gen + 1 },
					"",
					orig_url,
				);
			}
	...
	if (non_proto_url == "//rebuildtheuniverse.com/mjz_version") {
		...
		page_url ==
			"https://ssh.login.local:9342/mirror/rebuildtheuniverse.com/?type=mjz_version"
	) do_page_replace();
	else if (non_proto_url == "//rebuildtheuniverse.com/?type=real") {
		on_dom_load();
	} else if (
		page_url ===
			"https://ssh.login.local:9342/mirror/rebuildtheuniverse.com/?type=real"
	) on_dom_load();
	else if (
		page_url ===
			"https://ssh.login.local:9342/mirror/rebuildtheuniverse.com/?type=inject"
	) {
		document.stop = function () {};
		on_dom_load();
