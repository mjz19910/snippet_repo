...
(function () {
	...
	function l_log_if(level, ...args) {
		if (level <= local_logging_level) console.log(...args);
	}
	...
	class _TimerTagSingle {
		/** @readonly */
		value = TIMER_SINGLE;
	}
	class _TimerTagRepeating {
		/** @readonly */
	...
	class ReplyClearTypes {
		/** @type {import("./constant_types.ts").ReplyClearSingleT} */
	...
	class ReplySetTypes {
		...
		/** @type {import("./constant_types.ts").ReplySetRepeatingT} */
		repeating = ReplySetRepeating;
	}
	class ReplyTypes {
		/** @type {import("./constant_types.ts").WorkerDestroyTypeT} */
		destroy_worker = WorkerDestroyType;
		/** @type {import("./constant_types.ts").WorkerUpdateMessageHandlerReplyT} */
		update_handler = WorkerUpdateMessageHandlerReply;
		/** @type {import("./constant_types.ts").WorkerReadyReplyT} */
		ready = WorkerReadyReply;
		/** @type {import("./constant_types.ts").ReplyToWorkerStateT} */
		reply_to_local = ReplyToWorkerState;
		/** @type {import("./constant_types.ts").ReplyToLocalTimerT} */
		reply_to_main_timer = ReplyToLocalTimer;
		/** @type {import("./constant_types.ts").ReplyFromWorkerT} */
		from_worker = ReplyFromWorker;
		/** @type {import("./constant_types.ts").ReplyToWorkerT} */
		to_worker = ReplyToWorker;
		/** @type {import("./constant_types.ts").ReplySetTypesT} */
		set = new ReplySetTypes();
		/** @type {import("./constant_types.ts").ReplyClearTypesT} */
		clear = new ReplyClearTypes();
	}
	class TimeoutFireInfo {
		/** @type {import("./constant_types.ts").TimeoutFireSingleT} */
		single = TimeoutFireSingle;
		/** @type {import("./constant_types.ts").TimeoutFireRepeatingT} */
		repeating = TimeoutFireRepeating;
	}
	class TimeoutSetInfo {
		/** @type {import("./constant_types.ts").TimeoutSetSingleT} */
		single = TimeoutSetSingle;
		/** @type {import("./constant_types.ts").TimeoutSetRepeatingT} */
		repeating = TimeoutSetRepeating;
	}
	class TimeoutClearInfo {
		/** @type {import("./constant_types.ts").TimeoutClearSingleT} */
	...
	class WorkerReplyTimerFireTypes {
		...
		/** @type {import("./constant_types.ts").TimeoutRepeatingReplyT} */
		repeating = TimeoutRepeatingReply;
	}
	class WorkerReplyTypes {
		/** @type {import("./constant_types.ts").WorkerReplyTimerFireTypesT} */
		fire = new WorkerReplyTimerFireTypes();
	}
	class TimeoutWorkerTypes {
		/** @type {import("./constant_types.ts").WorkerReplyTypesT} */
		reply = new WorkerReplyTypes();
		/** @type {import("./constant_types.ts").WorkerUpdateMessageHandlerT} */
		update_message_handler = WorkerUpdateMessageHandler;
		/** @type {import("./constant_types.ts").TimeoutMessageReadyT} */
		ready = TimeoutMessageReady;
		/** @type {import("./constant_types.ts").TimeoutSetInfoT} */
		set = new TimeoutSetInfo();
		/** @type {import("./constant_types.ts").TimeoutClearInfoT} */
		clear = new TimeoutClearInfo();
		/** @type {import("./constant_types.ts").TimeoutSetTypesT} */
	...
	class TimerApi {
		/** @type {import("./constant_types.ts").WorkerAsyncMessageT} */
		async = WorkerAsyncMessage;
		/** @type {import("./constant_types.ts").TimeoutSetTypesT} */
		worker_set_types = TimeoutSetTypes;
		/** @type {import("./constant_types.ts").ReplyTypesT} */
		reply = new ReplyTypes();
		/** @type {import("./constant_types.ts").TimeoutFireInfoT} */
		fire = new TimeoutFireInfo();
		/** @type {import("./constant_types.ts").TimeoutWorkerTypesT} */
		worker = new TimeoutWorkerTypes();
		/** @type {import("./constant_types.ts").TimeoutSetStringsT} */
		set_names = new TimeoutSetStrings();
		/** @type {import("./constant_types.ts").TimeoutClearStringsT} */
		clear_names = new TimeoutClearStrings();
	}
	const g_timer_api = new TimerApi();
	...
	class Timer {
		...
		fire(tag, remote_id) {
			...
			const state = this.get_state_by_remote_id(remote_id);
			if (!state) throw new Error("No state for id");
			const active_state = this.activate_state(state);
			try {
		...
		activate_state(state) {
			...
			if (!this.m_active_state_map.has(id)) {
				if (typeof state.target_fn === "string") {
					const func = new Function(state.target_fn);
					const active_state = new ActiveTimerState(
						id,
						state.type,
						state.repeat,
						func,
			...
			const value = this.m_active_state_map.get(id);
		...
		send_worker_set_message(tag, remote_id, timeout) {
			...
			switch (tag) {
				case TIMER_SINGLE:
					msg_id = g_timer_api.worker.set.single;
					break;
				case TIMER_REPEATING:
					msg_id = g_timer_api.worker.set.repeating;
					break;
			}
			if (!msg_id) {
				console.assert(false, "Unknown timer_tag", tag);
				console.info(
					"TypeError like: let value:TIMER_SINGLE|TIMER_REPEATING (%o|%o) = %o",
					TIMER_SINGLE,
					TIMER_REPEATING,
					tag,
				);
				...
				case ReplyToLocalTimer:
					{
						console.assert(
							false,
							"on_result timer_result_msg needs a handler for ReplyMessage2",
						);
					}
					break;
				default:
					console.assert(
						false,
						"on_result timer_result_msg needs a handler for",
						msg,
					);
			}
		}
		/** @arg {TimeoutClearSingleMsg|TimeoutClearRepeatingMsg|_ReplySetRepeatingMsg|_ReplyClearSingleMsg|_ReplyClearRepeatingMsg} msg */
		on_reply(msg) {
			switch (msg.type) {
		...
		/** @arg {1|2} tag @arg {number} remote_id */
		force_clear(tag, remote_id) {
			this.assert_valid_worker();
			const state = this.get_state_by_remote_id(remote_id);
			if (!state) return;
			if (state.active) return this.clear(tag, remote_id);
		...
		destroy() {
			const api_map = this.m_api_map;
			window[g_timer_api.set_names.single] = api_map.get(
				g_timer_api.set_names.single,
			);
			window[g_timer_api.set_names.repeating] = api_map.get(
				g_timer_api.set_names.repeating,
			);
			window[g_timer_api.clear_names.single] = api_map.get(
				g_timer_api.clear_names.single,
			);
			window[g_timer_api.clear_names.repeating] = api_map.get(
				g_timer_api.clear_names.repeating,
			);
			for (const state_entry of this.remote_id_to_state_entries()) {
				const id = state_entry[0];
				void id;
				const state = state_entry[1];
				const active_state = this.activate_state(state);
	...
	class _WorkerReadyReplyMsg {
		/** @readonly */
		type = WorkerReadyReply;
		for_worker_state = true;
		value = {};
	}
	class _ReplySetSingleMsg {
		/** @readonly */
		type = ReplySetSingle;
		for_worker_state = true;
		value = {};
	}
	class _ReplySetRepeatingMsg {
		/** @readonly */
		type = ReplySetRepeating;
		value = {};
	}
	class _ReplyClearSingleMsg {
		/** @readonly */
		type = ReplyClearSingle;
		value = {};
	}
	class _ReplyClearRepeatingMsg {
		/** @readonly */
	...
	class WorkerState {
		...
		constructor(worker_code_blob, timer, executor_handle) {
			...
			if (!has_blob) {
				throw new Error(
					"WorkerState requires a blob with javascript code to execute on a worker",
				);
			}
		...
		init() {
			...
			this.worker.onmessage = function onmessage(e) {
				const worker_state = WorkerState.get_global_state();
				if (!worker_state) {
		...
		handle_message(e) {
			...
			switch (msg.type) {
				case TimeoutFireSingle:
					{
						this.timer.fire(TIMER_SINGLE, msg.value);
					}
					break;
				case TimeoutFireRepeating:
					{
						this.timer.fire(TIMER_REPEATING, msg.value);
					}
					break;
				case WorkerUpdateMessageHandler:
					break;
				case TimeoutMessageReady:
					this.on_result(msg);
					break;
				case TimeoutSetSingle:
					break;
				case TimeoutSetRepeating:
					break;
				...
				case ReplyToWorker:
					break;
				case TimeoutSingleReply:
					break;
				case TimeoutRepeatingReply:
					break;
				case TimeoutSetTypes:
					break;
				default:
					{
		...
		/** @arg {TimeoutMessageReadyMsg|_WorkerReadyReplyMsg|_ReplySetSingleMsg|ReplyToWorkerStateMsg|TimerWorkerSetTypesMsg} msg */
		on_result(msg) {
			...
			switch (msg.type) {
				case g_timer_api.worker.ready: {
					if (this.executor_handle === null || this.executor_handle.closed()) {
						console.assert(
							false,
							"WorkerState on_result called with invalid executor_handle",
						);
						break;
					}
		...
		/** @arg {TimeoutClearSingleMsg|TimeoutClearRepeatingMsg|TimerWorkerSetTypesMsg|_ReplyClearRepeatingMsg|_ReplyClearSingleMsg|_ReplySetRepeatingMsg|_ReplySetSingleMsg|_WorkerReadyReplyMsg|ReplyToWorkerStateMsg|ReplyToLocalTimerMsg|ReplyFromWorkerMsg} msg */
		...
		/** @arg {typeof _WorkerStateMessageV} msg */
		typedPostMessage(msg) {
			return this.worker?.postMessage(msg);
		}
		/** @arg {WorkerState} worker_state_value */
		static has_old_global_state_value(worker_state_value) {
			return this.has_global_state() &&
				!this.equals_global_state(worker_state_value);
		}
		/** @arg {WorkerState} worker_state_value */
		...
		destroy() {
			...
			if (this.executor_handle !== null && !this.executor_handle.closed()) {
				this.executor_handle.reject(
					new Error("Worker destroyed before it was connected"),
				);
			}
			this.connected = false;
			this.timer.destroy();
	...
	function set_timeout_on_remote_worker_executor(
		executor_accept,
		executor_reject,
	) {
		let failed = false;
		...
		try {
			worker_code_function(function (/** @type {_WorkerVars} */ verify_obj) {
				VERIFY(
					verify_obj.TIMER_REPEATING === TIMER_REPEATING,
					"TIMER_SINGLE constant matches",
				);
				VERIFY(
					verify_obj.TIMER_REPEATING === TIMER_REPEATING,
					"TIMER_REPEATING constant matches",
				);
				VERIFY(
					verify_obj.TIMER_TAG_COUNT === TIMER_TAG_COUNT,
					"TIMER_TAG_COUNT constant matches",
				);
				VERIFY(
					verify_obj.TimeoutSetTypes === TimeoutSetTypes,
					"TimerWorkerSetTypes constant matches",
				);
				return;
			});
			if (failed) return;
		...
		const executor_handle = new PromiseExecutorHandle(
			executor_accept,
			executor_reject,
		);
		const worker_code_blob = new Blob([
			"(",
			worker_code_function.toString(),
			")()",
			"\n//# sourceURL=$__.0",
		]);
		const worker_state = new WorkerState(
			worker_code_blob,
			timer,
			executor_handle,
		);
		...
		function remoteClearTimeout(id) {
			if (!worker_state) {
				self.clearTimeout = clearTimeout_global;
				l_log_if(LOG_LEVEL_WARN, "lost worker_state in timer");
		...
		function remoteClearInterval(id) {
			if (!worker_state) {
				self.clearInterval = clearInterval_global;
				l_log_if(LOG_LEVEL_WARN, "lost worker_state in timer");
	...
	/** @arg {{(arg0: _WorkerVars): void; }} verify_callback */
	function worker_code_function(verify_callback) {
				...
				case g_timer_api.worker.set.single:
					{
						...
						const local_id = remote_worker_state.set(
							TIMER_SINGLE,
							msg.remote_id,
							msg.timeout,
						);
		...
		class RemoteTimer {
			constructor() {
				/** @type {Map<number,_RemoteTimerState>} */
				this.m_remote_id_to_state_map = new Map();
			...
			/** @arg {number} remote_id */
			fire(remote_id) {
				const local_state = this.m_remote_id_to_state_map.get(remote_id);
			...
			set(tag, remote_id, timeout) {
				...
				switch (tag) {
					case TIMER_SINGLE:
						api_name = g_timer_api.set_names.single;
						break;
					case TIMER_REPEATING:
						api_name = g_timer_api.set_names.repeating;
						break;
				}
				if (!api_name) throw new Error("No api_name found");
				obj.local_id = globalThis[api_name](
					fire_timer,
					timeout,
					this,
					remote_id,
			...
			validate_tag(tag) {
				if (tag < TIMER_SINGLE || tag >= TIMER_TAG_COUNT) {
					console.assert(
						false,
						"Assertion failed in RemoteTimer.validate_tag: tag=%o is out of range",
					);
					console.info(
						"Info: range is TIMER_SINGLE to TIMER_TAG_COUNT-1 (%o...%o-1)",
						tag,
						TIMER_SINGLE,
						TIMER_TAG_COUNT,
					);
			...
			do_clear(msg) {
				const remote_id = msg.remote_id;
				const maybe_local_id = this.clear(remote_id);
				// debugger;
				switch (msg.type) {
					case g_timer_api.worker.clear.single:
						{
	...
	const move_timers_to_worker = new Promise(
		set_timeout_on_remote_worker_executor,
	);
