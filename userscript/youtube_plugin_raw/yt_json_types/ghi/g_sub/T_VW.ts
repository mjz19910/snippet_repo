type T_Tuple_1<T>=[T];
type T_PArr_1<T extends [any]>=["param_arr",T];
type T_PArr_R<T extends any[]>=["param_arr",T];
type T_VW<T,_tag=never>=T_PArr_1<[["child",Uint8Array,T]]>;
type T_VA_2<T,U>=[T,U];
type T_VW_3<Type,T>=[Type,T];
type T_VW_R<Type,T>=["raw",[Type,T]];
type TV_Str<T>=T_PArr_1<[["raw_child",Uint8Array,null,["string",T]]]>;
type T_VW_Bigint<T>=T_PArr_1<[["data64",number[],T]]>;
type T_FD32<T>=T_PArr_1<[["data_fixed32",T]]>;
type T_FD64<T extends bigint>=T_PArr_1<[["data_fixed64",T]]>;
type T_D32<T extends number>=T_PArr_1<[["data32",T]]>;
type T_Uint8Array<_len extends number>=Uint8Array;
