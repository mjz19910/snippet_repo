...
type T_InferTypedSplitOnce<WA extends string,S extends string,D extends string>=
	...
	? [WA,Rest]
	:Begin extends ""
	? T_InferredSplitOnce_NoBegin<WA,S,D>
	:Rest extends `${WA}`
	? [Begin,WA]
	:never
	:[S]
	;
;
type T_InferredSplitOnce_NoBegin<WA extends string,S extends string,D extends string>=
	S extends `${D}${infer Rest}`
	? T_InferTypedSplitOnce_NB<WA,Rest>
	:[S]
	;
;
type T_InferTypedSplitOnce_NB_1<WA extends string,Begin extends string,Rest extends string>=Rest extends `${WA}`? [Begin,WA]:never;
