...
type TI_SplitOnce_NE<WA extends string,S extends string,D extends string>=S extends `${infer Begin}${D}`? TI_SplitOnce_NB_1<WA,Begin>:S extends `${D}${WA}${infer Rest}`? ["",`${WA}${Rest}`]:[S];
