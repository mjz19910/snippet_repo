namespace MakeNumRange_Ext {
	type NextMakeNum_2<T>=T extends `${infer C extends number}`? `-${MakeNumRange.NextMakeNum_3<[],C>}` extends `${infer V extends number}`? V:never:never;
	...
	export type NumAddRes=MakeNumRange.NextMakeNum_3<[],3>;
