...
function init_module() {
	...
	class IndexedDBService extends BaseService {
		/** @returns {import("../yt_json_types/j/J_ResolverType.ts").J_ResolverType_Ready} */
		create_resolver() {
			return J_ResolverTypeImpl.make();
		}
		/** @template {"boxed_id"} K @arg {K} key @arg {IDBDatabase} db */
		create_store(key, db) {
			const obj_store = db.createObjectStore(key, { keyPath: "key" });
		...
		cache_index() {
			return this.store_cache_index;
		}
		/** @type {Set<string>} */
		database_diff_keys = new Set();
		/** @private @type {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData[]} */
		committed_data = [];
		/** @private @type {import("../yt_json_types/d/mod_D/_T/DA_CacheInfoType.ts").DA_CacheInfoType} */
		store_cache = [];
		cache() {
			return this.store_cache;
		}
		/** @type {string[][]} */
		_delayed_log_messages = [];
		/** @type {Promise<import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData[]>[]} */
		waiting_promises = [];
		...
		/** @temporary @template {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData} T @arg {T} x @arg {number} version @returns {Promise<T|null>} */
		put_box(x, version) {
			return this.ht.put("boxed_id", x, version);
		}
		/** @arg {number} version @returns {Promise<import("../yt_json_types/ghi/group_G.ts").DST_LoadId|null>} */
		async get_load_id(version) {
			/** @type {import("../yt_json_types/ghi/group_G.ts").DST_LoadId|null} */
			const box = await this.get("boxed_id", "boxed_id:load_id", version);
			return box;
		}
		/** @arg {number} version @returns {Promise<import("../yt_json_types/ghi/group_G.ts").DST_SaveId|null>} */
		async get_save_id(version) {
			/** @type {import("../yt_json_types/ghi/group_G.ts").DST_SaveId|null} */
		...
		async load_store_from_database(store, version) {
			/** @type {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData[]} */
			let boxed;
			try {
			...
			for (const item of boxed) await this.load_store(store, item, version);
			this.has_loaded_keys = true;
			this.on_loaded_resolver.resolve();
		}
		/** @arg {StoreData} store @arg {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData} item @arg {number} version */
		...
		/** @template {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData} T @arg {T} x @arg {number} version @returns {Promise<T>} */
		...
		async save_database(store, version) {
			...
			if (cur_id_box.z[0] !== this.expected_save_id) {
				this.expected_save_id = cur_id_box.z[0];
			}
		...
		async save_store_data_to_database(store, version) {
			const s_values = store.stores.values();
			for (const store of s_values) {
				await this.save_store_to_database(store, version);
			}
		}
		/** @arg {import("../zb_plugin_types/Support_Service.ts").StoreDescription<string>} store @arg {number} version */
		async save_store_to_database(store, version) {
			const results = await Promise.allSettled(
				store.data.map((item) =>
					this.save_store_item_to_database(store, item, version)
				),
			);
		...
		/** @template T @arg {import("../yt_json_types/m/make_item_group.ts").make_item_group<T>} x @arg {T[]} _mt */
		uv_unpack_mt(x, _mt) {
			/** @type {import("../yt_json_types/m/MT_MakeSplitObj.ts").MT_MakeSplitObj<T>} */
			const make = {}, nul = null;
			/** @type {{u:import("../yt_json_types/m/make_item_group.ts").make_arr_t<T>|import("../yt_json_types/m/make_item_group.ts").make_instance_name_t<string>|import("../yt_json_types/m/make_item_group.ts").make_many_t<T>|import("../yt_json_types/m/make_item_group.ts").make_one_t<T>|import("../yt_json_types/m/make_item_group.ts").make_typeof_name_t<T>}} */
			const D_holder = {};
			D_holder.u = x;
			make.arr = nul;
			make.instance_name = nul;
			make.many = nul;
			make.one = nul;
			make.typeof_name = nul;
			switch (x.l) {
				default:
					{
						/** @type {unknown} */
						const ux = x;
						if (
							typeof ux === "object" && ux !== null && "k" in ux &&
							typeof ux.k === "string"
						) {
		...
		/** @template T @arg {import("../yt_json_types/m/make_item_group.ts").make_item_group<T>} x */
		uv_unpack(x) {
		...
		/** @template T @arg {import("../yt_json_types/m/make_item_group.ts").make_item_group<T>} x @returns {boolean} @arg {import("../yt_json_types/stu/group_T.ts").T_GetTypeof<T>} ty */
		is_vi_typeof_check(x, ty) {
			/** @arg {T} x @arg {import("../yt_json_types/stu/group_T.ts").T_GetTypeof<T>} ty */
			function t_check_typeof(x, ty) {
			...
			switch (x.l) {
				...
				case "one": {
					const z1 = x.z;
					return t_check_typeof(z1[0], ty);
				}
				case "arr": {
					const u = x.z, z2 = u[0];
					return z2.length === 0 || t_check_typeof(z2[0], ty);
				}
				case "many": {
					const b = x.z, m = b[0], z3 = m[0];
					return m.length === 0 || z3.length === 0 || t_check_typeof(z3[0], ty);
				}
			}
		}
		/** @arg {TypedIndexedDB} typed_db @arg {"boxed_id"} key @arg {number} version */
		async open_rw_object_store(typed_db, key, version) {
			const db_request = this.get_db_request(version);
			if (!db_request) throw new Error("Unable to access IndexedDB");
			const db = await this.get_async_result(db_request);
			const s = this.open_transaction_scope(typed_db, db, key, "readwrite");
			return typed_db.objectStore(s.tx, key);
		}
		/** @arg {"boxed_id"} key @arg {string} query @arg {number} version */
		async delete(key, query, version) {
			const typed_db = new TypedIndexedDB();
			const obj_store = await this.open_rw_object_store(typed_db, key, version);
			return this.get_async_result(obj_store.delete(query));
		}
		/** @api @public @template {import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[U]} T @template {"boxed_id"} U @arg {U} key @arg {T} value @arg {number} version */
		...
		async putImpl(key, value, version) {
			...
			if (this.db_wait_promise) {
				const wait_result = await this.db_wait_promise;
				if (wait_result.type === "failure") return null;
				return value;
			}
			const resolver = J_ResolverTypeImpl.make();
		...
		get_db_request(version) {
			try {
				const db_req = indexedDB.open("yt_plugin", version);
		...
		/** @arg {TypedIndexedDB} typed_db @arg {IDBDatabase} db @template {"boxed_id"} U @arg {U} key @arg {IDBTransactionMode} mode @returns {import("../zb_plugin_types/TypedIDBTransactionScope.ts").TypedIDBTransactionScope} */
		open_transaction_scope(typed_db, db, key, mode) {
			...
			const s = {
				...
				tx,
				typed_db,
				/** @type {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[U]>|null} */
				obj_store: null,
		...
		/** @arg {import("../zb_plugin_types/TypedIDBTransactionScope.ts").TypedIDBTransactionScope} scope @arg {Event} event */
		...
		assert_assume_is(x, y) {
			if (x !== y) throw new Error();
		}
		/**
		 * @param {TypedIndexedDB} tdb @template {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData} T @arg {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<T>} store @arg {T} value
		...
		/** @template {"boxed_id"} U @arg {{error_count:number;db:IDBDatabase;tx:IDBTransaction|null;obj_store:import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[U]>|null;typed_db:TypedIndexedDB;}} s @arg {import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[U]} value */
		async force_update(s, value) {
			if (!s.obj_store) throw new Error("No object store");
			const put_req = this.start_put_request(s.typed_db, s.obj_store, value);
			if (put_req.type === "err") {
				throw new AggregateError([put_req.err], "start error");
			}
			try {
				const ret = await this.get_async_result(put_req.req);
				return ret;
			} catch (e) {
		...
		async open_database(key, version) {
			...
			const db = await this.get_async_result(this.get_db_request(version));
			this.database_opening = false;
			this.database_open = true;
			const typed_db = new TypedIndexedDB();
			const tx_scope = this.open_transaction_scope(
				typed_db,
				db,
				key,
				"readwrite",
			...
			const d_cache = this.cache();
			const no_null_cache = d_cache.filter((e) =>
				e !== null && "type" in e && !this.ht.loaded_keys.has(e.key)
			);
			/** @type {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData[]} */
			const new_arr = [];
			/** @type {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData[]} */
			...
			/** @type {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData[]} */
			const unknown_arr = [];
			/** @arg {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData} x @arg {"new"|"same"|"diff"|"unknown"} changed */
			const commit_value = (x, changed) => {
				switch (changed) {
					case "new":
						new_arr.push(x);
						break;
				...
				const idx = d_cache.indexOf(x);
				d_cache[idx] = null;
			};
			try {
				for (const x_value of d_cache) {
					...
					if (this.ht.loaded_keys.has(x_value.key)) {
						const y_value = this.ht.loaded_map.get(x_value.key);
						y_value;
					}
					const cursor_req = typed_db.openCursor(
						s.obj_store,
						TypedIDBValidKeyS.only(x_value.key),
					);
					if (tx_scope.is_tx_complete) {
					...
					if (cur_cursor === null) {
						if (this.log_db_actions) {
							console.log("[db_cursor.done]", cur_cursor);
						}
						if (this.log_db_actions) {
						...
						const put_req = typed_db.put(s.obj_store, x_value);
					...
					if (this.log_db_actions) {
						console.log("[db_cursor.continue]", cur_cursor, y_value);
					}
				...
				const complete_event = await tx_scope.complete_promise;
		 ...
		 * @param {K} key
		 * @template {"boxed_id"} K
		 * @template {import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]} T
		...
		async get(key, store_key, version) {
			...
			/** @type {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<T>} */
			const obj_store = typed_db.objectStore(tx, key);
			const result = await this.get_async_result(
				typed_db.get(obj_store, store_key),
			);
			return result;
		}
		...
		async getAll(key, version) {
			...
			const tx = typed_db.transaction(db, key, "readonly");
			const obj_store = typed_db.objectStore(tx, key);
			const result = await this.get_async_result(typed_db.getAll(obj_store));
			return result;
		}
		/** @api @public @arg {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData} obj */
		push_value(obj) {
			const { key } = obj;
			const idx = this.add_to_index(key, obj);
			if (this.log_cache_push) console.log("push wait", key, idx, obj);
		}
		/** @arg {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData["key"]} key @arg {import("../yt_json_types/ghi/group_G.ts").G_BoxedDatabaseData} x */
		add_to_index(key, x) {
			const cache_arr = this.cache();
			const cache_index = this.cache_index();
		...
		onUpgradeNeeded(request, event) {
			if (this.log_all_events) {
				console.log("IDBOpenDBRequest: oldVersion", event.oldVersion);
			}
			const { result: db, transaction: tx } = request;
			if (!tx) throw new Error("No transaction");
			this.x.get("handle_types").indexed_db_createDatabaseSchema(
				this,
				event.oldVersion,
				db,
			);
		...
		async get_async_result(req) {
			const res = await this.get_async_result_impl(req);
			if (res[0] === "error") {
				const [, event, error] = res;
		...
		async database_diff(version) {
			...
			const tx = typed_db.transaction(ret.db, "boxed_id", "readonly");
			ret.store = typed_db.objectStore(tx, "boxed_id");
			ret.store_data = await this.get_async_result(typed_db.getAll(ret.store));
			ret.store_diff = this.get_diff_by_key(
				this.database_diff_keys,
				ret.store_data,
			);
			return ret;
		}
		async database_diff_console_example() {
			const [set_p] = await Promise.allSettled([(() => {
				const yt_plugin = window.yt_plugin;
				if (!yt_plugin) throw new Error();
				const idb = yt_plugin.indexed_db;
...
/** @type {import("../yt_json_types/j/J_ResolverType.ts").J_ResolverTypeHelpers['assume_changed_state']} */
function assume_changed_state(_cls, _state) {
	return true;
}
/** @type {import("../yt_json_types/j/J_ResolverType.ts").J_ResolverTypeHelpers['change_state']} */
...
class J_ResolverTypeImpl {
	...
	get as_ready() {
		/** @returns {import("../yt_json_types/j/J_ResolverType.ts").J_ResolverType} */
		const get_any_cls = () => this;
		const cls = get_any_cls();
...
class TypedIndexedDB {
	...
	transaction(db, storeNames, mode) {
		return db.transaction(storeNames, mode);
	}
	/** @template {"boxed_id"} K @arg {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]>} obj_store @arg {import("../zb_plugin_types/TypedIDB.ts").TypedIDBValidKey<import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]["key"]>|import("../zb_plugin_types/TypedIDB.ts").TypedIDBKeyRange<import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]["key"]>} [query] @arg {IDBCursorDirection} [direction] @returns {IDBRequest<import("../zb_plugin_types/TypedIDB.ts").TypedIDBCursorWithValue<import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]>|null>} */
	...
	/** @arg {IDBTransaction} tx @arg {K} key @template {"boxed_id"} K @template {import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]} T @returns {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<T>} */
	objectStore(tx, key) {
		const rq = tx.objectStore(key);
		return as(rq);
	}
	/** @template {"boxed_id"} K @template {import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]} T @arg {T["key"]} key @arg {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<T>} store @returns {IDBRequest<T|null>} */
	get(store, key) {
		return store.get(key);
	}
	/** @template {{}} T @arg {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<T>} store @returns {IDBRequest<T[]>} */
	getAll(store) {
		return store.getAll();
	}
	/** @template {"boxed_id"} K @template {import("../yt_json_types/d/mod_D/_T/DT_DatabaseStoreTypes.ts").DT_DatabaseStoreTypes[K]} T @arg {T} value @arg {import("../zb_plugin_types/TypedIDB.ts").TypedIDBObjectStore<T>} store @returns {IDBRequest<IDBValidKey>} */
	put(store, value) {
...
class TypedIDBValidKeyS {
	/** @template {IDBValidKey} T @arg {T} key @returns {import("../zb_plugin_types/TypedIDB.ts").TypedIDBValidKey<T>} */
	static only(key) {
		return { type: "key", key };
	}
}
