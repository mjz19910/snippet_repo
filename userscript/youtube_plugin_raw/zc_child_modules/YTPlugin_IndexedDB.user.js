	...
	put_boxed_pl(version,...args) {
		switch(args[1]) {
			...
			case "MP": {
				...
				/** @type {DST_Browse_MP} */
				const z2={...this.mk_s3(type,tag,value),w: "/key/a/b/j/w/z"};
	...
	put_boxed_id_3(version,...s0) {
		...
		switch(k) {
			...
			case "guide_entry_id": /*db*/ {
				...
				switch(x.c) {
					...
					case "PL": {
						...
						const z={a: "ST:D",b: "boxed_id",j: tag,key: `boxed_id:${tag}:${iv.c}:${this.za2(iv.z[1])}`,z: [x]};
						return {args: s0,promise: this.put_box(z,version)};
					}
					case "UC": {
						let iv=x.z[0];
			...
			case "playlist_id": {
				switch(s0[1]) {
					...
					case "PL": {
						...
						const kj={key: `boxed_id:${tag}:${id}:${this.za2(value.z[1])}`,j: `${tag}:${id}`};
			...
			case "hashtag_id": {
				let [tag,,value]=s0;
				/** @type {DST_HashtagId} */
				const z=this.make_box_3(tag,this.za2(value),value);
				let promise=this.put_box(z,version); return {args: s0,promise};
			}
			case "channel_id": {
				let [tag,,value]=s0;
				/** @type {DST_Channel_UC} */
				const z=this.make_box_4(tag,value.c,this.za2(value.z[1]),value);
				let promise=this.put_box(z,version); return {args: s0,promise};
			}
			case "video_time": {
				let [tag,,value]=s0;
			...
			case "browse_id": {
				switch(s0[1]) {
					case "FE": {
						...
						/** @type {DST_Browse_FE} */
						const z={
							key: `boxed_id:${tag}:${id}:${this.za2(value.z[1])}`,
							a: "ST:D",b: "boxed_id",j: `${tag}:${id}`,w: "/key/a/b/j/w/z",z: [value]
						};
						let promise=this.put_box(z,version); return {args: s0,promise};
					}
					case "MP": {
