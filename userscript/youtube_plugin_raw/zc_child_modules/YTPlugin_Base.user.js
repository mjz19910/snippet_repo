...
const { do_export, get_exports, required, as } = require(
...
//#endregion
//#region constants
/** @private @type {import("../support_2/YtdAppElement.ts").YtdAppElement} */
 ...
 */
function assume_assert_is_instanceof(x, c_ty) {
	x;
	c_ty;
}
/**
...
class CustomEventTarget {
	/** @private @type {import("../yt_json_types/j/J_CustomEventTargetEvents.ts").J_CustomEventTargetEvents} */
	_events = {};
	/** @api @public @arg {import("../yt_json_types/j/J_CustomEventTargetEvents.ts").J_HandlerInfoArgs} args */
	addEventListener(...args) {
		let [type, handler] = args;
		let ht1 = this._events[type];
		if (ht1) {
			/** @type {NonNullable<import("../yt_json_types/j/J_CustomEventTargetEvents.ts").J_CustomEventTargetEvents[keyof import("../yt_json_types/j/J_CustomEventTargetEvents.ts").J_CustomEventTargetEvents]>[number][]} */
			let handlers = ht1;
	...
	/** @api @public @arg {import("../zb_plugin_types/types.ts").CustomEventType} event */
	dispatchEvent(event) {
		let msg_arr = this._events[event.type];
		if (!msg_arr) return;
		for (let i = 0; i < msg_arr.length; i++) {
			/** @type {(this:CustomEventTarget,event:import("../zb_plugin_types/types.ts").CustomEventType) => void} */
...
class DomObserver extends CustomEventTarget {
	...
	wait_ports = new Set();
	/** @private @type {Map<MessagePort,import("../yt_json_types/d/group_D.ts").D_ResState[]>} */
	...
	wait_for_port(port, cur_count) {
		...
		return new Promise((accept) => {
			...
			if (this.port_to_resolvers_map.has(port)) {
				if (this.port_to_resolvers_map.has(port)) {
					this.port_to_resolvers_map.get(port)?.push(state);
				}
			} else this.port_to_resolvers_map.set(port, [state]);
		});
	}
	trace = false;
...
function do_find_video() {
	...
	} else {if (async_plugin_init.__debug) {
...
function on_ytd_app(element) {
	...
	if (is_yt_debug_enabled || is_ytd_app_debug_enabled) {
	...
	ytd_app.addEventListener("yt-navigate-finish", function (event) {
		...
		if (!target_element) {
			throw new Error("Missing ytd-page-manager when we have ytd-app");
		}
...
/** @private @arg {import("../support_1/AsyncPluginInitEvent.ts").AsyncPluginInitEvent} event */
...
/**
 * @typedef {{type: "ready";detail: {current_page: import("../support_2/YtCurrentPage.ts").YtCurrentPage;ytd_page_manager: YtdPageManagerElement;};}} FindElement_IsReady
 */

/**
 * @typedef {{type:"done"}|{type:"dom-not-ready"}|{type:"current-page-not-found";detail:{ytd_page_manager: YtdPageManagerElement}}|FindElement_IsReady} MaybeReadyEvent
 */

/**
 * @param {MaybeReadyEvent} ready_event
...
async function find_ytd_watch_flexy(ready_event, iter_count) {
	...
	if (!current_page.__theater_handler_plugin_attached) {
		current_page.addEventListener(
			"yt-set-theater-mode-enabled",
			update_ui_plugin,
		);
		current_page.__theater_handler_plugin_attached = true;
	}
	if (is_yt_debug_enabled) {
		console.log(
			"PageManager:current_page:" + current_page.tagName.toLowerCase(),
		);
	}
	if (current_page.tagName.toLowerCase() != "ytd-watch-flexy") {
		if (iter_count > 100) {
			console.log(
				"found current_page [%s] at iter=%o",
				current_page.tagName.toLowerCase(),
				iter_count,
			);
		}
	...
	found_element_count++;
	on_ytd_watch_flexy(current_page);
	return;
}
/**
 * @param {import("../support_1/AsyncPluginInitEvent.ts").AsyncPluginInitEvent} event
 * @param {number} iter_count
 * @param {DomObserver} obj
 * @param {number} cur_count
 * @returns {Promise<import("../yt_json_types/ghi/dnp_wrong/IterationDecision.ts").IterationDecision>}
 */
async function find_from_ytd_app(event, iter_count, obj, cur_count) {
	iterate_find_element(
		ytd_page_manager,
		"ytd-page-manager",
		document,
		on_ytd_page_manager,
	...
	iterate_find_element(
		...
		"yt-playlist-manager",
		document,
		event.detail.elements.on_yt_playlist_manager,
	);
	const find_event = create_find_event_for_ytd_watch_flexy();
	const res = iterate_find_element_async(
		find_event,
		find_ytd_watch_flexy,
		iter_count,
	);
	...
	if (!box_map.has("video-list")) return { type: "continue" };
	if (ytd_page_manager === null) return { type: "continue" };
	if (!ytd_page_manager.getCurrentPage()) return { type: "continue" };
...
function create_find_event_for_ytd_watch_flexy() {
	...
	if (!current_page) {
		return { type: "current-page-not-found", detail: { ytd_page_manager } };
	}
...
async function async_plugin_init(event) {
	...
	try {
		while (true) {
			...
			x:
			if (
				plugin_state.show_interesting_elements && plugin_state.polymer_loaded &&
				document.body
			) {
				let interesting_body_elements = [
					...make_iterator(document.body.children),
				].filter((e) => {
					let e_tn = e.tagName;
					...
					if (
						e.id === "watch-page-skeleton" && (
							e.classList[0] === "watch-skeletonhidden" ||
							e.classList[0] === "watch-skeleton"
						)
					) return false;
					if (e.id === "player" && e.classList.value === "skeleton flexy") {
						return false;
					}
					if (
						e.id === "watch7-content" && e.classList.value === "watch-main-col"
					) return false;
				...
				if (
					ytd_app && interesting_body_elements.includes(ytd_app) &&
					interesting_body_elements.length === 1
				) break x;
			...
			iterate_find_element(ytd_app, "ytd-app", document, on_ytd_app);
			// END(ytd-app): obj.dispatchEvent({type: "ytd-app",detail,port});
			// BEGIN(ytcp-app): dispatchEvent({type: "find-ytd-app"});
			iterate_find_element(ytcp_app, "ytcp-app", document, on_ytcp_app);
			// END(ytcp-app): obj.dispatchEvent({type: "ytcp-app"});
			...
			if (ytd_app) {
				let do_break = await find_from_ytd_app(
					event,
					iter_count,
					obj,
					cur_count,
...
class VolumeRange {
	...
	static create_if_needed() {
		...
		if (!ytd_app.volume_range && audio_gain_controller) {
			...
			let volume_range = new VolumeRange(
				0,
				100 * 5,
				100 * 5 * 2,
	...
	onKeyDown(event) {
		...
		if (event.key == "f") {
			var compressor_reduction_factor =
				this.gain_controller.dynamics_compressor.reduction;
			...
			new_gain = (new_gain * -1) /
				(Math.log(this.max_compressor_reduction * -1) * -1 / 2);
...
class R_HandleRichGrid_Base {
	...
	/** @handler @public @arg {string} path @arg {import("../yt_json_types/d/group_D.ts").Todo_D_RichGrid} renderer */
	richGridRenderer(path, renderer) {
		...
		if (renderer.contents) {
			...
			let filtered = this.rendererContentItemArray.replace_array(
				renderer.contents,
			);
...
class MyReader {
	...
	read_any(size, pos) {
		...
		if (size === void 0) this.cur_len = this.len;
	...
	read_any_impl() {
		this.failed = false;
		/** @private @type {import("../yt_json_types/d/group_D.ts").D_ProtobufWireFormat[]} */
		let data = [];
		let loop_count = 0;
		let log_slow = true;
		for (; this.pos < this.cur_len; loop_count++) {
			...
			if (!log_slow && loop_count % 4096 == 0) {
				console.log(
					"taking a very long time to read protobuf data",
					loop_count / 4096 | 0,
				);
			}
		}
		/** @private @type {import("../yt_json_types/d/group_D.ts").D_ProtobufObj[]} */
		let res_arr = [];
	...
	indexOutOfRange(writeLength) {
		return RangeError(
			"index out of range: " + this.pos + " + " + (writeLength || 1) + " > " +
				this.len,
		);
	}
	fixed32() {
	...
	skipTypeEx(fieldId, wireType) {
		...
		switch (wireType) {
			...
			case 4:
				...
				break;
			case 5:
				first_num.push(["data_fixed32", fieldId, this.fixed32()]);
				break;
			default:
				break;
		}
...
const base64_url_dec = new Base64Binary(
...
/** @private @type {unknown[]} */
let mk_tree_arr = [];
function act_found_create_yt_player(
	/** @private @type {{ data: { type: string; data: [unknown, unknown, unknown]; }; }} */ event,
) {
	...
	function plr_raw_replace(
		...
		if (plr_raw_replace_debug) {
			console.log(
				"plr_raw_replace",
				"::",
				"args.raw_player_response.playerAds=",
		...
		if (plr_raw_replace_debug) {
			console.log(
				"plr_raw_replace",
				"::",
				"args.raw_player_response.adPlacements=",
	...
	if (
		tr != "yt.player.Application.createAlternate" &&
		tr != "yt.player.Application.create"
	) return;
...
function new_pv_fn(val, cc, /** @private @type {unknown[]} */ ...args) {
	...
	win_watch.dispatchEvent({
		type: "new_window_object",
		data: {
			type: cc.value_tr,
			data: [cc.function_value, val, args, act_cb_obj],
		},
	});
	if (!act_cb_obj.fired && cc.function_value) {
		ret = cc.function_value.apply(val, args);
	} else ret = act_cb_obj.ret;
	return ret;
}
/** @private @arg {MKState} cc */
function on_mk_function_property(cc) {
	/** @this {{}} */
	function with_this(/** @private @type {unknown} */ ...args) {
...
function mk_run(cc) {
	if (
		locked_set.has(cc.target) &&
		locked_set.get(cc.target).names.indexOf(cc.property_key) > -1
	) return cc;
	...
	} else locked_set.set(cc.target, { names: [cc.property_key] });
	return cc;
}
...
class YtdPageManagerElement extends HTMLElement {
	/** @returns {import("../support_2/YtCurrentPage.ts").YtCurrentPage|undefined} */
	getCurrentPage() {
		throw new Error();
	}
}
...
function on_ytd_watch_flexy(element) {
	...
	ytd_watch_flexy.addEventListener("yt-navigate", function (event) {
		for (let handler of on_yt_navigate) handler(event);
	});
}
/** @private @type {string[]} */
...
function is_watch_page_active() {
	...
	return ytd_page_manager.getCurrentPage()?.tagName.toLowerCase() ===
		"ytd-watch-flexy";
}
...
let box_map = new Map();
/** @private @type {import("../support_2/YtdPlayerElement.ts").YtdPlayerElement|null} */
let ytd_player = null;
/** @private @arg {HTMLElement} element */
...
class YTNavigateFinishEvent {
	...
	/** @api @public @type {import("../yt_json_types/ghi/group_G.ts").G_NavFinishDetail} */
	detail = as({});
}
...
/** @private @arg {import("../zb_plugin_types/types.ts").AsyncPluginEventDetail["handle_types"]} handle_types @arg {import("../zb_plugin_types/types.ts").AsyncPluginEventDetail["elements"]} elements */
...
function _close_div_scope() {
	...
	function title_text_overlay_update() {
		if (title_text_overlay_enabled) {
			...
			if (title_on) overlay_content_div.style.display = "";
			else overlay_content_div.style.display = "none";
		} else {
	...
	function activate_nav() {
		...
		ytd_page_manager.addEventListener("yt-page-type-changed", function () {
			...
			if (
				ytd_page_manager.getCurrentPage()?.tagName.toLowerCase() !=
					"ytd-watch-flexy"
			) {
	...
	function update_plugin_overlay_location() {
		...
		function sumOffset(element) {
			...
			for (;;) {
				...
				if (next_element instanceof HTMLElement) cur_element = next_element;
				else break;
			}
			return cache;
		}
	...
	function on_yt_action(event) {
		if (!("detail" in event)) {
			console.log(
				"[on_yt_action] event [yt-action] has no detail (not a CustomEvent)",
			);
			return;
		}
		let { detail } = event;
...
export_((exports) => {
	exports.no_storage_access = no_storage_access;
});
...
class AudioGainController {
	...
	constructor() {
		...
		this.dynamics_compressor = this.initCompressor(
	...
	attach_element_list(media_node_list) {
		for (let i = 0; i < media_node_list.length; i++) {
			...
			let media_element_source = this.audioCtx.createMediaElementSource(
				video_element,
			);
...
Object.__ia_excludeKeysS = function (
	/** @private @type {{ [s: string]: unknown; }|ArrayLike<unknown>} */ target,
	/** @private @type {string} */ ex_keys_str,
) {
	/** @private @type {unknown} */
...
let volume_plugin_style_element = createStyleElement(
	volume_plugin_style_source,
);
...
function yt_plugin_base_main() {
	...
	override_prop(
		window,
		"getInitialData",
		new PropertyHandler(do_proxy_call_getInitialData),
	);
	...
	/** @private @arg {[()=>import("../yt_json_types/ghi/group_G.ts").G_NavFinishDetail["response"], object, []]} apply_args */
	...
	function log_page_type_change(event) {
		...
		if (!ytd_page_manager) {
			const target_element = get_html_elements(document, "ytd-page-manager")[0];
			if (!target_element) throw new Error("Missing ytd_page_manager");
			else on_ytd_page_manager(target_element);
		}
		if (!ytd_page_manager) throw new Error("Invalid state");
		yt_handlers.on_page_type_changed(detail);
		let page_manager_current_tag_name = ytd_page_manager.getCurrentPage()
			?.tagName.toLowerCase();
		let nav_load_str =
			`page_type_change: {current_page_element_tagName: "${page_manager_current_tag_name}", pageType: "${detail.pageType}"}`;
...
/** @private @template {string} X @arg {X} x @template {string} S @arg {S} s @returns {import("../yt_json_types/stu/group_T.ts").T_Split<X,string extends S?",":S>} */
function split_string(x, s = as(",")) {
	if (!x) debugger;
	let r = x.split(s);
	return as(r);
}
/** @private @arg {WA|null} _wa @template {string} WA @template {string} S @arg {S} s @template {string} D @arg {D} d @returns {import("../yt_json_types/_rtv_wrong/T_InferTypedSplitOnce.ts").TI_SplitOnce<WA,S,D>} */
...
/** @private @arg {A1|null} _a1 @template {string} A1 @arg {A2|null} _a2 @template {string} A2 @template {string} S @arg {S} s @template {string} D @arg {D} d @returns {import("../yt_json_types/_rtv_wrong/T_InferTypedSplitOnce.ts").TI_SplitOnce_2arg<A1,A2,S,D>} */
...
/** @private @arg {WA|null} _wx @template {string} S @template {string} WA @arg {S} s @template {string} D @arg {D} d @returns {import("../yt_json_types/_rtv_wrong/T_InferTypedSplitOnce.ts").TI_SplitOnce_NE<WA,S,D>} */
...
class ApiBase2 {
	...
	/** @public @template {{}} T @arg {T} obj @returns {import("../yt_json_types/stu/group_T.ts").T_DistributedKeysOf<T>} */
	get_keys_of(obj) {
		if (!obj) debugger;
	...
	/** @protected @template {{}} T @arg {T} obj @returns {import("../yt_json_types/stu/group_T.ts").T_DistributedKeysOf_2<T>} */
	get_keys_of_2(obj) {
		if (!obj) debugger;
...
class ApiBase extends ApiBase2 {
	...
	/** @protected @template {string} T @arg {T} t @returns {import("../yt_json_types/stu/group_T.ts").TP_ParseUrlSearchParams<T>} */
	...
	/** @protected @template {string} T @arg {T} t @returns {import("../yt_json_types/stu/group_T.ts").TP_KeyofSearchParams<T>} */
...
class ServiceWithResolver extends ApiBase {
	...
	/** @protected @returns {import("../zb_plugin_types/types.ts").ServiceResolverValue} */
	get x() {
		return as_any(this.#x.value);
	}
	/** @arg {()=>void} cb */
...
class PrivateAccessorCache extends ServiceWithResolver {
	get codegen() {
		return this.x.get("codegen");
	}
	get common_methods() {
		return this.x.get("common_methods");
	}
	get handle_types() {
	...
	get save_db() {
		return this.x.get("save_db");
	}
	get service_methods() {
		return this.x.get("service_methods");
	}
	get x_methods() {
		return this.x.get("x_methods");
	}
	get x_Renderer() {
		return this.x.get("x_Renderer");
	}
}
class ServiceWithAccessors extends ServiceWithResolver {
	/** @param {import("../zb_plugin_types/types.ts").ServiceResolverBox<{}>} x */
	constructor(x) {
		super(x);
		...
		this.addOnServicesListener(() => {
			...
			this.ix = this.accessor.indexed_db;
			this.ps = this.accessor.parser_service;
			this.sd = this.accessor.save_db;
			this.sm = this.accessor.service_methods;
			this.xm = this.accessor.x_methods;
			this.xr = this.accessor.x_Renderer;
		});
	}
}
...
class BaseService extends ServiceWithMembers {
	/** @public @arg {K} k @template U @template {import("../yt_json_types/stu/group_T.ts").T_DistributedKeyof<T>} K @template {{[U in string]:{};}} T @arg {T} x @arg {(this:this,x:T[K])=>U} f */
	H_s(k, x, f) {
		let tm = this;

		x:
		if (!(tm instanceof ServiceData)) {
	...
	/** @public @template U @arg {import("../yt_json_types/abc/group_C.ts").CF_T_GM} cf @template T @arg {{sendPost: true;apiUrl: T;}} x @arg {(this:this,x:T)=>U} f */
	T_GM(cf, x, f) {
		const { sendPost, apiUrl, ...y } = this.s(cf, x);
		this.g(y); /*#destructure_done*/
		if (sendPost !== true) debugger;
		return f.call(this, apiUrl);
	}
	/** @api @public @template {{}} T @arg {string|import("../yt_json_types/abc/group_C.ts").CF_M_s} cf @arg {T} x */
	s(cf, x) {
		if (!x) debugger;
		this.sm.k(cf, x);
		return x;
	...
	y(cf, k, x, f) {
		return f.call(this, this.sm.w(cf, k, x), `${cf}.${k}`);
	}
	/**
	...
	//#endregion
	//#region make
	/** @arg {string} k @template T @arg {T} x @returns {import("../yt_json_types/m/make_item_group.ts").make_one_t<T>} */
	make_one_t(k, x) {
		const b = "item", c = "one";
		return { a: "/di/a/k/l/m/z", k: b, l: c, m: k, z: [x] };
	}
	/** @arg {string} k @template T @arg {T[]} x @returns {import("../yt_json_types/m/make_item_group.ts").make_arr_t<T>} */
	make_arr_t(k, x) {
		const b = "item", c = "arr";
		return { a: "/di/a/k/l/m/z", k: b, l: c, m: k, z: [x] };
	}
	//#endregion
	//#region save
	/** @public @arg {import("../yt_json_types/abc/group_C.ts").CF_M_k|string} k @arg {{}} x */
	k(k, x) {
		this.save_primitive(k, x);
	}
	...
	save_enum(cf, ns, k) {
		return this.sd.save_enum_impl(cf, ns, k);
	}
	//#region accessors
	//#endregion
	...
	make_prim_num(x) {
		return { a: "primitive", e: "number", z: [x] };
	}
	/** @template {string} L @template T @arg {L} l @arg {T} x @returns {import("../yt_json_types/d/mod_D/DI_T/DI_T_move.ts").T_DI_Raw<L,T>} */
	make_DI_Raw(l, x) {
		return { a: this.get_KZ("kl"), k: "raw", l, z: [x] };
	}
	/** @template T @arg {T} x @returns {import("../yt_json_types/stu/group_T.ts").T_GetPrimitiveTag<T>} */
	get_primitive_tag(x) {
		switch (typeof x) {
			case "bigint":
				return as("bigint");
	...
	/** @template {string|number} Z @param {Z} z @returns {import("../yt_json_types/d/mod_D/DI_T/DI_T_move.ts").T_PrimitiveBox<Z>} */
	make_prim_v(z) {
		return { a: this.get_KZ("k"), k: this.get_primitive_tag(z), z: [z] };
	}
	/** @template T @arg {T} x @returns {import("../yt_json_types/d/mod_D/DI_T/DI_T_move.ts").T_PrimitiveBox<T>} */
	make_Typeof(x) {
		return { a: this.get_KZ("k"), k: this.get_primitive_tag(x), z: [x] };
	}
	/** @template T @arg {T} x @returns {import("../yt_json_types/d/mod_D/DI_T/DI_T_move.ts").DIT_Box_Typeof2<import("../yt_json_types/stu/group_T.ts").T_GetPrimitiveTag<T>,T>} */
	make_BoxTypeof(x) {
		return { a: this.get_KZ("k"), k: this.get_primitive_tag(x), z: [x] };
	}
	/** @template T @arg {T} v @returns {import("../yt_json_types/d/mod_D/DI_T/DI_T_move.ts").MK_DIInfo1<T>} */
	make_obj_input(v) {
		return { z: [v] };
	}
	/** @template {number} T @param {T} x @returns {import("../yt_json_types/d/mod_D/DI_T/DI_T_move.ts").T_PrimitiveBox<T>} */
	make_prim_num_t(x) {
		return { a: this.get_KZ("k"), k: this.get_primitive_tag(x), z: [x] };
	}
	//#endregion
	/** @public @template {string} X @arg {X} x @template {string} S @arg {S} s @returns {X extends infer X1?import("../yt_json_types/stu/group_T.ts").T_Split<X1,string extends S?",":S>:never} */
	split_str(x, s = as(",")) {
		if (!x) debugger;
		let r = x.split(s);
		return as(r);
	}
	/** @public @template {string[]} X @arg {X} x @template {string} S @arg {S} s @returns {import("../support_1/Join.ts").Join<X,S>} */
	join_string(x, s) {
		if (!x) debugger;
		let r = x.join(s);
		return as(r);
	}
	/** @protected @template {`https://${string}`|`http://${string}`} T @arg {T} str @returns {import("../support_1/url_parse/UrlParse.ts").UrlParse<T>} */
	...
	/** @public @template {string} T @template {string} U @arg {T} str @arg {U} ends_str @returns {x is (T extends `${infer B}${infer R}`?`${B}${import("../yt_json_types/stu/group_T.ts").Some<R>}${string}${U}`:`${string}${U}`)} */
	...
	/** @private @template {string} T @template {string} U @arg {T} x @arg {U} sep @returns {import("../yt_json_types/stu/group_T.ts").T_SplitOnce<T,U>[number]} */
	...
	/** @private @template {{}} T @arg {T extends Record<string, never>?T:{} extends T?import("../yt_json_types/stu/group_T.ts").T_DistributedKeysOf<T> extends []?T:never:never} x */
	on_empty_object(x) {
		if (!x) {
			debugger;
			return;
	...
	z_cf(cf, x, f) {
		...
		return this.z(x, (x, i) => f.call(this, cf, x, i));
	}
	/** @protected @template {{}} T @arg {T extends Record<string, never>?T:{} extends T?import("../yt_json_types/stu/group_T.ts").T_DistributedKeysOf<T> extends []?T:never:never} x */
	g(x) {
		this.on_empty_object(x);
	}
	/** @public @template {{}} T @arg {({} extends T?import("../yt_json_types/stu/group_T.ts").T_DistributedKeysOf<T> extends []?T:never:never)|null|undefined} x */
	tg_base(x) {
		this.t_base(x, this.g);
	}
	...
	tf = (x) => (y) => this.tv(x, y);
	//#region T_Optional
	/** @template T @arg {T} x @returns {import("../yt_json_types/stu/group_T.ts").Some<T>} */
	some(x) {
		return { type: "s", v: x };
	}
	/** @template T @arg {T} x @returns {import("../yt_json_types/stu/group_T.ts").Some<T>} */
	...
	/** @template T @arg {import("../yt_json_types/stu/group_T.ts").Some<T>} x */
	mw(x) {
		/** @template ST @template {this} CU */
		class SomeEx {
			/** @arg {CU} cls @arg {import("../yt_json_types/stu/group_T.ts").Some<ST>} x */
			constructor(cls, x) {
				this.cls = cls;
				this.some = x;
			}
	...
	/** @template T @arg {import("../yt_json_types/stu/group_T.ts").Some<T>} x @returns {T} */
	mu(x) {
		return x.v;
	}
	/** @arg {(this:this,x:T)=>U} y @template T @arg {import("../yt_json_types/stu/group_T.ts").Some<T>} x @template U @returns {import("../yt_json_types/stu/group_T.ts").Some<U>} */
	mt(x, y) {
		return this.m(y.call(this, x.v));
	}
	/** @arg {(this:this,x:T)=>U} y @template {{}} T @arg {import("../yt_json_types/stu/group_T.ts").Some<T|null|undefined>} x @template U @returns {import("../yt_json_types/stu/group_T.ts").Some<U|null>} */
	m_t(x, y) {
		return this.m(this.t_base(x.v, y));
	}
	/** @template {{}|undefined} T @arg {import("../yt_json_types/stu/group_T.ts").Some<T>} x @returns {T extends infer V?V extends undefined?import("../yt_json_types/stu/group_T.ts").None:import("../yt_json_types/stu/group_T.ts").Some<V>:never} */
	m_ud(x) {
		/** @arg {import("../yt_json_types/stu/group_T.ts").Some<T>|import("../yt_json_types/stu/group_T.ts").None} x @returns {asserts x is T extends infer V?V extends undefined?import("../yt_json_types/stu/group_T.ts").None:import("../yt_json_types/stu/group_T.ts").Some<V>:never} */
		function assume_ret(x) {
			x;
		}
		/** @type {import("../yt_json_types/stu/group_T.ts").Some<T>|import("../yt_json_types/stu/group_T.ts").None} */
		let mr;
	...
	ms_ud(x) {
		return this.m_ud(this.m(x));
	}
	/** @template {{}|null} T @arg {import("../yt_json_types/stu/group_T.ts").Some<T>} x @returns {T extends infer V?V extends null?import("../yt_json_types/stu/group_T.ts").None:import("../yt_json_types/stu/group_T.ts").Some<V>:never} */
	m_nu(x) {
		/** @arg {import("../yt_json_types/stu/group_T.ts").Some<T>|import("../yt_json_types/stu/group_T.ts").None} x @returns {asserts x is T extends infer V?V extends null?import("../yt_json_types/stu/group_T.ts").None:import("../yt_json_types/stu/group_T.ts").Some<V>:never} */
		function assume_ret(x) {
			x;
		}
		/** @type {import("../yt_json_types/stu/group_T.ts").Some<T>|import("../yt_json_types/stu/group_T.ts").None} */
	...
	m_nu_t(x, y) {
		...
		/** @type {import("../yt_json_types/stu/group_T.ts").Some<U>|import("../yt_json_types/stu/group_T.ts").None} */
		let mr;
		if (x.type === "n") {
			mr = { type: "n" };
		} else {
	...
	/** @arg {(this:this,x:T)=>U} y @template {{}} T @template {import("../yt_json_types/stu/group_T.ts").Some<T[]>} Opt @arg {Opt} x @template U */
	mz(x, y) {
		return this.mt(x, (x) => this.z(x, y));
	}
	/** @arg {(this:this,x:T)=>U} f @template T @arg {import("../yt_json_types/stu/group_T.ts").Some<T>} m @template U */
	mb(f, m) {
		return this.mt(m, f);
	}
	/** @template T @arg {T} x @template U @arg {(this:this,x:T)=>U} y @returns {import("../yt_json_types/stu/group_T.ts").Some<U>} */
	ms(x, y) {
		return this.mt(this.m(x), y);
	}
	/** @template {{}} T @arg {T|undefined} x @template U @arg {(this:this,x:T)=>U} y @returns {import("../yt_json_types/stu/group_T.ts").Some<U|null>} */
	ms_t(x, y) {
		return this.ms(x, (x) => this.t_base(x, y));
	}
	/** @template {{}} T1 @arg {import("../yt_json_types/stu/group_T.ts").Some<T1|undefined>} x @template {{}} T2 @arg {(this:this,x:T1)=>T2} y @template T3 @arg {(this:this,x:T2)=>T3} z @returns {import("../yt_json_types/stu/group_T.ts").Some<T3|null>} */
	m2_t(x, y, z) {
		return this.m_t(this.m_t(x, y), z);
	}
	/** @template {{}} T1 @arg {T1|undefined} x @template {{}} T2 @arg {(this:this,x:T1)=>T2} y @template T3 @arg {(this:this,x:T2)=>T3} z @returns {import("../yt_json_types/stu/group_T.ts").Some<T3|null>} */
	ms2_t(x, y, z) {
		return this.m2_t(this.m(x), y, z);
	}
	/** @template {{}} T @arg {import("../yt_json_types/stu/group_T.ts").Some<T|null>} x @template U @arg {(this:this,x:T)=>U} y @returns {import("../yt_json_types/stu/group_T.ts").Some<U|null>} */
	mt_t(x, y) {
		return this.mt(x, (x) => this.t_base(x, y));
	}
	/** @template {string} T_CF @arg {T_CF} cf @arg {(this:this,cf:T_CF,x:T)=>U} f @template T @arg {import("../yt_json_types/stu/group_T.ts").Some<T>} m @template U @returns {import("../yt_json_types/stu/group_T.ts").Some<U|null>} */
	mt_cf(m, cf, f) {
		return this.mt(m, (x) => this.t_cf(cf, x, f));
	}
	//#endregion
	/** @private @template T @template {T} U @arg {T} a @arg {import("../yt_json_types/n/NoInfer.ts").NoInfer<U>} b */
	_cq_no_infer(a, b) {
		if (a !== b) debugger;
	}
	...
	/** @public @type {<T,U extends T>(a:T,b:import("../yt_json_types/n/NoInfer.ts").NoInfer<U>)=>void} */
	cq = this._cq_no_infer;
}
/** @typedef {{t:YtHandlers;path:string}} ApiIterateState */
class YtObjectVisitor {
	/** @handler @public @arg {ApiIterateState} state @arg {import("../yt_json_types/abc/AD.ts").AD_AppendContinuationItems} action */
	appendContinuationItemsAction(state, action) {
		if (!action.continuationItems) debugger;
		let filtered = state.t.handlers.renderer_content_item_array.replace_array(
			action.continuationItems,
		);
		if (filtered.length > 0) action.continuationItems = filtered;
	}
	/** @handler @public @arg {ApiIterateState} state @arg  {import("../yt_json_types/d/group_DC.ts").DC_ReloadContinuationItems} command */
	reloadContinuationItemsCommand({ t: state }, command) {
		...
		let filtered = state.handlers.renderer_content_item_array.replace_array(
			iterable_items,
		);
		if (filtered.length > 0) command.continuationItems = filtered;
	}
	/** @handler @public @template {{}} T1 @template T2,T3  @arg {ApiIterateState} state @arg {import("../yt_json_types/stu/group_T.ts").TD_ItemSection_3<T1,T2,T3>} renderer */
	itemSectionRenderer_with_state(state, renderer) {
		...
		renderer.contents = renderer.contents.filter(
			state.t.filter_renderer_contents_item,
			state.t,
		);
	}
	/** @handler @public @arg {ApiIterateState} state @arg {import("../yt_json_types/d/group_D.ts").Todo_D_RichGrid} renderer */
...
class IterateApiResultBase extends BaseService {
	/** @constructor @public @arg {import("../zb_plugin_types/types.ts").ServiceResolverBox<{}>} x @arg {YtObjectVisitor} obj_visitor */
...
class YtHandlers extends BaseService {
	...
	filter_renderer_contents_item(item) {
		...
		for (let key of keys) {
			...
			console.log(
				"filter_handlers: new item section at itemSectionRenderer.contents[]: ",
				key,
			);
		}
		return true;
	}
	/** @constructor @public @arg {import("../zb_plugin_types/types.ts").ServiceResolverBox<{}>} res */
	constructor(res) {
		super(res);
		...
		this.blacklisted_item_sections = new Map([
			...
			[
				...
				true, /*compact promoted video (is_ads=true)*/
			],
			...
			[
				...
				true, /*promoted sparkles web (is_ads=true)*/
			],
			["radioRenderer", false],
	...
	/** @api @public @arg {import("../support_2/FetchJsonParseArgs.ts").FetchJsonParseArgs} target_args  */
	on_handle_api(target_args) {
		...
		/** @private @type {import("../yt_json_types/d/group_D.ts").D_ApiUrlFormat} */
		let api_url = as(parsed_url.href);
		let url_type = this.sm.decode_url(api_url);
		if (url_type === "s.i.youtube_outline.bars_3.v2.24px.svg") {
			throw new Error("Invalid url_type");
		}
		let res = this.sm.decode_json_response(url_type, parsed_obj);
		if (!res) {
			console.log("Decoding of the json response did not return a result");
			return;
		}
		this.x.get("x_GenericApi").G_ResponseTypes(response, res);
		this.iteration.default_iter({ t: this, path: url_type }, parsed_obj);
	}
	known_page_types = split_string(
		"settings,watch,browse,shorts,search,channel,playlist",
		",",
	);
	do_initial_data_trace = false;
	/** @private @arg {import("../yt_json_types/r/group_R.ts").G_RS_ByPageType} x */
	plugin_run_on_data(x) {
		/** @arg {import("../yt_json_types/r/group_R.ts").G_RS_ByPageType} x */
			...
			case "browse":
				{
					x:
					if ("rootVe" in x) {
						if (x.rootVe !== 3854) break x;
						ok(x);
						break;
					}
					x:
					if ("rootVe" in x) {
						if (x.rootVe !== 6827) break x;
						ok(x);
						break;
					}
					x:
					if ("rootVe" in x) {
						if (x.rootVe !== 96368) break x;
						ok(x);
						break;
					}
					x: {
						if (!this.sm.is_EP_Val(x, 3854)) break x;
						ok(x);
						break;
					}
					x: {
						if (!this.sm.is_EP_Val(x, 6827)) break x;
						ok(x);
						break;
					}
					x: {
						if (!this.sm.is_EP_Val(x, 96368)) break x;
						ok(x);
						break;
					}
					debugger;
	...
	/** @api @public @arg {[()=>import("../yt_json_types/ghi/group_G.ts").G_NavFinishDetail["response"], object, []]} apply_args */
	on_initial_data(apply_args) {
		/** @private @type {import("../yt_json_types/ghi/group_G.ts").G_NavFinishDetail["response"]} */
		...
		if (this.do_initial_data_trace) {
			...
			this.upgrade_obj(ret.endpoint, { is_initial_endpoint: true })
				.is_initial_endpoint = true;
		}
	...
	/** @api @public @arg {import("../yt_json_types/ghi/group_G.ts").G_NavFinishDetail} detail */
...
class HandleRendererContentItemArray extends BaseService {
	...
	/** @private @arg {import("../yt_json_types/r/group_R.ts").R_RichItem} content_item */
	filter_for_rich_item_renderer(content_item) {
		...
		if ("adSlotRenderer" in renderer.content) {
			if (noisy_logging) {
				console.log("adSlotRenderer=", renderer.content.adSlotRenderer);
			}
			return false;
		}
	...
	/** @private @arg {import("../yt_json_types/r/group_R.ts").R_RichSection} content_item */
	handle_rich_section_renderer(content_item) {
		let renderer = content_item.richSectionRenderer;
		/** @private @type {import("../yt_json_types/ghi/group_G.ts").G_RichSection} */
	...
	replace_array(arr) {
		return as(
			arr.filter((/** @private @type {typeof arr[number]} */ content_item) => {
...
class CsiService extends BaseService {
	/** @private @type {(import("../yt_json_types/stu/group_T.ts").T_RidFormat<string>)[]} */
	rid_keys = [
		"GetAccountAdvanced_rid",
		"GetAccountBilling_rid",
		"GetAccountDownloads_rid",
		"GetAccountMenu_rid",
		"GetAccountNotifications_rid",
		"GetAccountOverview_rid",
		"GetAccountPlayback_rid",
		"GetAccountPrivacy_rid",
		"GetAccountSharing_rid",
		"GetAccountsList_rid",
	...
	/** @private @arg {{key:import("../yt_json_types/stu/group_T.ts").T_RidFormat<string>;value:`0x${string}`}} x */
	...
	/** @private @arg {{key:import("../yt_json_types/stu/group_T.ts").T_RidFormat<string>;value:`0x${string}`}} param */
	...
	/** @private @type {{[x: import("../yt_json_types/stu/group_T.ts").T_RidFormat<string>]: `0x${string}`|undefined;}} */
	rid = {};
	/** @constructor @public @arg {import("../zb_plugin_types/types.ts").ServiceResolverBox<{}>} x */
	constructor(x) {
		super(x);
		this.data = {
			/** @private @type {import("../yt_json_types/d/group_D.ts").D_BrowseEndpointPages|null} */
			yt_fn: null,
			/** @private @type {import("../yt_json_types/r/group_R.ts").RC_CsiServiceC["value"]|null} */
			c: null,
			/** @private @type {import("../yt_json_types/r/group_R.ts").RC_CsiVarTypes["cver"]|null} */
			cver: null,
			/** @private @type {"1"|null} */
			yt_li: null,
			/** @private @type {"1"|null} */
	...
	/** @private @arg {import("../yt_json_types/d/group_D.ts").D_BrowseEndpointPages} value */
	verify_param_yt_fn(value) {
		switch (value) {
			...
			case "what_to_watch":
				return true;
			default:
				console.log("[verify_param_bad]", value);
				debugger;
				return false;
		}
	}
	/** @api @public @arg {import("../yt_json_types/d/group_D.ts").DRC_CsiVarKV[]} params */
	on_params(params) {
		for (let param of params) {
			switch (param.key) {
				...
				case "yt_ad":
					...
					continue;
				case "yt_fn":
					if (!this.verify_param_yt_fn(param.value)) debugger;
					this.data[param.key] = param.value;
					continue;
			}
...
class GFeedbackService extends BaseService {
	data = {
		...
		context: null,
		/** @private @type {import("../yt_json_types/stu/group_S.ts").SP_GFeedbackServiceRouteParam["value"]|null} */
		route: null,
	};
	get handle_types() {
		return this.x.get("handle_types");
	...
	/** @private @arg {Extract<import("../yt_json_types/r/group_R.ts").ToKeyValue<import("../yt_json_types/stu/group_S.ts").SP_GFeedbackVarMap>,{key:"e"}>} param */
	parse_e_param(param) {
	...
	/** @api @public @arg {{context: import("../yt_json_types/d/group_D.ts").D_ContextTypeStr|null;}} data_target @arg {import("../yt_json_types/d/group_D.ts").D_ContextTypeStr} x */
	on_context_param(data_target, x) {
		data_target.context = x;
		switch (x) {
			case "channel_creator":
				return;
			case "yt_web_remix_unlimited":
				return;
			case "yt_web_search":
				return;
			case "yt_web_unknown_form_factor_kevlar_w2w":
				return;
			case "yt_web_unlimited":
				return;
			default:
		}
		switch (x) {
	...
	/** @api @public @arg {import("../yt_json_types/stu/group_S.ts").SP_GFeedbackServiceParams[]} params */
	on_params(params) {
		for (let param of params) {
			switch (param.key) {
				case "browse_id_prefix":
					if (param.value !== "") debugger;
					break;
				case "browse_id":
					this.sm.browseId(param.value);
					break;
				case "context":
					this.on_context_param(this.data, param.value);
					break;
				case "e":
					this.parse_e_param(param);
					break;
				case "has_alc_entitlement":
					break;
				case "has_unlimited_entitlement":
					break;
				case "ipcc":
					break;
				case "is_alc_surface":
				...
				case "is_viewed_live":
					break;
				case "has_premium_lite_entitlement":
					break;
				case "logged_in":
					{
						if (param.value == "0") {
						...
						if (param.value == "1") {
							general_service_state.logged_in = true;
							break;
						}
						debugger;
					}
				...
				case "premium_membership":
					if (param.value !== "non_member") debugger;
					general_service_state.premium_membership = param.value;
					break;
				case "route":
					{
					...
					break;
				default:
					console.log("[param_key]", param);
					debugger;
			}
			this.maybe_new_e();
		}
	}
	/** @private @arg {import("../yt_json_types/stu/group_S.ts").SP_GFeedbackServiceRouteParam} x */
	parse_route_param(x) {
		let h = this.ps;
...
class GuidedHelpService extends BaseService {
	...
	/** @api @public @arg {import("../yt_json_types/stu/group_S.ts").SP_GuidedHelp_SPs["params"]} params */
	on_params(params) {
		for (let param of params) {
			switch (param.key) {
				case "logged_in":
					...
					break;
				case "context":
					this.x.get("g_feedback_service").on_context_param(
						this.data,
						param.value,
					);
					break;
				default:
					console.log("[new_param_key]", param);
					debugger;
			}
		}
	}
...
class TrackingServices extends BaseService {
	/** @private @arg {import("../yt_json_types/r/group_R.ts").RC_Csi_SPs} service */
	on_csi_service(service) {
		this.x.get("csi_service").on_params(service.params);
	}
	/** @private @arg {import("../yt_json_types/r/group_R.ts").RC_ECatcher_SPs} service */
	on_e_catcher_service(service) {
		this.x.get("e_catcher_service").on_params(service.params);
	}
	/** @private @arg {import("../yt_json_types/r/group_R.ts").RC_GFeedback_SPs} service */
	on_g_feedback_service(service) {
		this.x.get("g_feedback_service").on_params(service.params);
	}
	/** @private @arg {import("../yt_json_types/stu/group_S.ts").SP_GuidedHelp_SPs} service */
	...
	/** @private @arg {import("../yt_json_types/r/group_R.ts").RC_GoogleHelp_SPs} service */
	...
	/** @api @public @arg {import("../yt_json_types/ghi/group_G.ts").GRC_ServiceTrackingParams} service_arg */
	set_service_params(service_arg) {
		switch (service_arg.service) {
			case "CSI":
				this.on_csi_service(service_arg);
				break;
			case "ECATCHER":
				this.on_e_catcher_service(service_arg);
				break;
			case "GFEEDBACK":
				this.on_g_feedback_service(service_arg);
				break;
			case "GUIDED_HELP":
				this.on_guided_help_service(service_arg);
				break;
			case "GOOGLE_HELP":
				this.on_google_help_service(service_arg);
				break;
			default:
				debugger;
		}
...
class ModifyEnv extends BaseService {
	...
	modify_global_env() {
		...
		function handle_json_parse(
			...
			if (is_yt_debug_enabled) {
				console.log("handle_json_parse", request, options);
			}
			let ret;
			try {
				if (onfulfilled) ret = onfulfilled(response_text);
				else ret = response_text;
			} catch (err) {
			...
			return {
				...
				then(onfulfilled, onrejected) {
					return result.result.then(
						bind_promise_handler(input, onfulfilled, onrejected),
					);
				},
		...
		class R_Fake {
			...
			constructor(handlers, response) {
				this.__handlers__ = handlers;
				this.__response__ = response;
			}
			text() {
				return this.__handlers__.text();
			}
			get redirected() {
				return this.__response__.redirected;
			}
			get ok() {
				return this.__response__.ok;
			}
			get status() {
				return this.__response__.status;
			}
			get headers() {
				return this.__response__.headers;
			}
			get statusText() {
				return this.__response__.statusText;
			}
			get type() {
				return this.__response__.type;
			}
			get url() {
				return this.__response__.url;
			}
			get body() {
				return this.__response__.body;
			}
			get bodyUsed() {
				return this.__response__.bodyUsed;
			}
			arrayBuffer() {
				return this.__response__.arrayBuffer();
			}
			blob() {
				return this.__response__.blob();
			}
			formData() {
				return this.__response__.formData();
			}
			json() {
				return this.__response__.json();
			}
			/** @returns {Response} */
			clone() {
					...
					get(obj, key, _proxy) {
						switch (key) {
							...
							case "clone":
								return obj[key];
							default:
								console.log("[new_response_key] [%s]", key);
								debugger;
						}
						return obj[key];
					},
		...
		function fetch_promise_handler(request, options, response) {
			let fake_res = new R_Fake({
				text() {
					...
					return handle_fetch_response_2({ input: { request, options } }, {
		...
		function fetch_inject(user_request, request_init) {
			...
			if (request_init) {
				if (
					request_init.method === "HEAD" &&
					request_init.signal instanceof AbortSignal
				) break x;
			}
			if (
				typeof user_request === "string" &&
				user_request.startsWith("https://www.gstatic.com")
			...
			let ret_1 = ret.then(
				fetch_promise_handler.bind(null, user_request, request_init),
				fetch_rejection_handler,
			);
			return ret_1;
		}
		let t = this;
		...
		navigator.sendBeacon = function (...args) {
			if (
				typeof args[0] === "string" && args[0].indexOf("/api/stats/qoe") > -1
			) return true;
...
class ServiceData extends BaseService {
	/** @protected @type {import("../yt_json_types/ghi/group_G.ts").GA_FormatItagNum[]} */
	format_itag_arr = [
		18,
		133,
		134,
	...
	/** @protected @type {import("../yt_json_types/d/group_D.ts").QualArr} */
...
class JsonReplacerState {
	/** @constructor @public @arg {import("../yt_json_types/D_JsonReplacerArgs.ts").D_JsonReplacerArgs} args */
	constructor(args) {
		this.object_count = 0;
		this.cur_cf = args.cf;
...
export_((exports) => {
	exports.ServiceWithResolver = ServiceWithResolver;
});
export_((exports) => {
	exports.ServiceWithAccessors = ServiceWithAccessors;
});
export_((exports) => {
