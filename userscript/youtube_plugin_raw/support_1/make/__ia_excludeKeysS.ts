...
export function __ia_excludeKeysS<T extends string,U extends {},C extends T_Split<T,",">>(target: U,ex_keys_str: T_Split<T,","> extends unknown[]? T:never): {[I in Exclude<keyof U,C[number]>]: U[I];} {
	...
	for(;i<ex_keys.length;i++) {
		{
			key=ex_keys[i];
			let {[key]: _,...rest_}=obj as {[x: string]: unknown;};
			rest=rest_;
		};
		obj=rest;
	};
